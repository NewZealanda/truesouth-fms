...
function _avMilfordGroup(sess,ym){
  ...
  (sess||[]).forEach(function(s){
    if(String(s.startTimeLocal||'').slice(0,7)!==ym)return;
    if(String(s.productName||'').trim().toLowerCase()!=='milford sound fly cruise fly')return;
    if(+s.seats>=900)return;
    var t=String(s.startTimeLocal||'').slice(11,16);if(t)times[t]=1;
  });
  var cols=[];Object.keys(times).sort().forEach(function(t){
    cols.push({label:'FCF '+t,name:'Milford Sound Fly Cruise Fly',time:t});
    cols.push({label:'FLB '+(_FB_RETURN[t]||t),name:'Milford Sound One-Way Flight',time:t});
  });
  return {group:'Milford',time:'',cols:cols};
}
...
function renderAvailabilityView(){
  ...
  var _mil=_avMilfordGroup(sess,ym);
  var colgroups=[_AV_MTCOOK].concat(_mil.cols.length?[_mil]:[]).concat([_AV_FRANZ]);
  ...
  var _totCols=colgroups.reduce(function(x,g){return x+g.cols.length;},0);
  h+='<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12.5px;min-width:'+(120+_totCols*54)+'px">'+
    '<thead>'+
      '<tr style="color:var(--text2)"><th rowspan="2" style="text-align:left;padding:7px 8px;position:sticky;left:0;background:var(--card);z-index:2;vertical-align:bottom;font-size:11px;text-transform:uppercase;letter-spacing:.04em">Day</th>'+
        colgroups.map(function(g){return '<th colspan="'+g.cols.length+'" style="text-align:center;padding:6px;font-weight:800;font-size:12px;border-left:2px solid var(--border2);border-bottom:1px solid var(--border2)">'+_avEsc(g.group)+(g.time?' <span style="font-weight:400;color:var(--text3)">'+_avEsc(g.time)+'</span>':'')+'</th>';}).join('')+'</tr>'+
      '<tr style="color:var(--text3);font-size:10px;text-transform:uppercase;letter-spacing:.03em">'+
        colgroups.map(function(g){return g.cols.map(function(c,ci){return '<th style="text-align:center;padding:5px 6px;white-space:nowrap;font-weight:600'+(ci===0?';border-left:2px solid var(--border2)':'')+'">'+_avEsc(c.label)+'</th>';}).join('');}).join('')+'</tr>'+
    '</thead><tbody>';
  days.forEach(function(dt){
    ...
    h+='<tr style="border-top:1px solid var(--border2)"><td style="padding:7px 8px;white-space:nowrap;font-weight:700;position:sticky;left:0;background:var(--card)">'+_avEsc(dlab)+'</td>'+
      colgroups.map(function(g){return g.cols.map(function(c,ci){return _cell(_avSeatsProd(sess,dt,c.name,c.time),ci===0);}).join('');}).join('')+'</tr>';
  });
