...
window.savePerson=async function(){
  ...
  if(_canEditCrew&&(crewId||finalName)){
    ...
    if(d.photo) lsSet('ts_crew_photo_'+crewId,d.photo);
    {
      ...
      if(!fullR){
        // Full upsert failed (a NEWER column may be missing on this DB). Save the safe base, then patch
        // each newer column ON ITS OWN so one missing column can't drop the others — the birthday (dob)
        // was being lost because it shared a write with type_ratings.
        const minR=await sbU('ts_crew',[{id:crewId,name:finalName,weight:parseFloat(d.w)||null,
          endorsements:JSON.stringify(d.endorse||[]),code:d.code||'',dl_num:d.dlNum||'',
          caa_license:d.caaNum||'',medical_expiry:d.medExpiry||null,oca_due:d.ocaDue||null}]);
        const dobR=await sbU('ts_crew',[{id:crewId,dob:d.dob||null}]);
        await sbU('ts_crew',[{id:crewId,type_ratings:JSON.stringify(d.typeRatings||[])}]);
        await sbU('ts_crew',[{id:crewId,first_aid:d.firstAid||null,avsec_expiry:d.avsecExpiry||null}]);
        if(d.photo) await sbU('ts_crew',[{id:crewId,photo:d.photo}]);
        if(!minR) toast('Warning: profile may not have saved to server — check connection','warn');
        else if(d.dob&&!dobR) toast('Saved — but the birthday could not be stored on the server. The ts_crew.dob column is missing: apply the flightduty.sql migration.','warn');
      }
    }
  }
...
// Combined grid columns: one checkbox controls several underlying permissions (set together, shown
// ticked only when ALL are on). Keeps the grid compact while the app still reads the granular perms.
var _PERM_COMBOS={
  pilotbag:  ['pilotbag','flightrecord','flightduty'],            // Pilot Bag = Flight Record + Logbooks + F&D
  roster_leave:['roster','leave'],                                // Roster & Leave
  roster_mgr:['roster_edit','leave_approve','reports_to','pay_week'] // Roster Mgr = edit + approvals + reports-to + pay week
};
// A single perm's effective value for the grid display (honours the "on by default" perms).
function _permEffState(over,base,k){
  if(over[k]!==undefined)return !!over[k];
  if(base[k]!==undefined)return !!base[k];
  return (k==='settings'||k==='pilotbag'||k==='monitoring'||k==='visitors'||k==='ops_notices');
}
...
window.toggleRolePerm=function(role,perm,val){
  ...
  (_PERM_COMBOS[perm]||[perm]).forEach(function(k){S.rolePerms[role][k]=val;});
...
function renderAdminPerms(){
  ...
  var PERM_GROUPS=[
    ...
    {cat:'Crew', col:'#22c55e', perms:[
      {k:'pilotbag',      lbl:'Pilot Bag',   keys:['pilotbag','flightrecord','flightduty'], tip:'Pilot Bag — Flight Record, Logbooks and Flight & Duty (one toggle grants all three)'},
      ...
      {k:'roster_leave',  lbl:'Roster & Leave', keys:['roster','leave'], tip:'View the roster and submit/view own leave requests'},
      {k:'roster_mgr',    lbl:'Roster Mgr',  keys:['roster_edit','leave_approve','reports_to','pay_week'], tip:'Edit the roster & build patterns, approve leave, Reports-to org chart, and the pay-week view'}
    ]},
  ...
  ROLE_ROWS.forEach(function(row,ri){
    ...
    PERM_COLS.forEach(function(col){
      var eff;
      if(col.keys){
        // Combined column — ticked only when ALL underlying perms are on.
        eff=col.keys.every(function(k){return _permEffState(over,base,k);});
      } else if(col.k==='settings'||col.k==='monitoring'||col.k==='visitors'||col.k==='ops_notices'){
