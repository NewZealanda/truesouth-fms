...
// Searchable haystack for a saved loadsheet: aircraft, PIC, co-pilot, route, date (ISO + d/m/y + pretty), time, passenger + infant names.
function _lsHaystack(s){
  var f=(s&&s.form)||{};
  var names=Object.keys(f.names||{}).map(function(k){return f.names[k];}).join(' ');
  var inf=Object.keys(f.infantNames||{}).map(function(k){return f.infantNames[k];}).join(' ');
  var dmy='';if(f.date){try{var p=String(f.date).split('-');if(p.length===3)dmy=p[2]+'/'+p[1]+'/'+p[0]+' '+p[2]+'/'+p[1];}catch(e){}}
  return [f.ac,f.pic,f.coPilot,f.dep,f.dest,f.etd,f.date,dmy,_fmtFlightDate(f.date),names,inf].join(' ').toLowerCase();
}
// AND-match: every whitespace-separated token must be present (e.g. "sla milford" or "smith 30/06").
function _lsSearchMatch(s,q){
  if(!q)return true;
  var hay=_lsHaystack(s);
  return q.split(/\s+/).every(function(tok){return !tok||hay.indexOf(tok)>=0;});
}
function _savedSearchBox(ph){
  return '<input id="savedSearchBox" type="text" placeholder="'+ph+'" value="'+String(S.savedSearch||'').replace(/"/g,'&quot;')+'" oninput="S.savedSearch=this.value;render()" style="width:100%;box-sizing:border-box;margin-bottom:12px;padding:9px 12px;background:var(--card2);border:1px solid var(--border2);border-radius:8px;color:var(--text);font-size:13px">';
}
function renderSaved(){
  ...
  // Counts (saved manifests removed v28.59 — loadsheets only)
  const binLs=S.saved.filter(s=>s.status==='deleted');
  const binCount=binLs.length;
  ...
  // ── Bin tab ──
  if(tab==='bin'){
    const _binLsF=search?binLs.filter(s=>_lsSearchMatch(s,search)):binLs;
    return tabBar+`<div style="display:flex;justify-content:flex-end;align-items:center;gap:10px;margin-bottom:10px;flex-wrap:wrap">
      ${binCount>0?`<button class="btn btn-red" style="font-size:12px" onclick="window.emptyBin()">🗑️ Empty Bin</button>`:''}
    </div>`+
    (binCount?_savedSearchBox('Search bin — PIC, passenger, date, time, aircraft…'):'')+
    (!binCount
      ?`<div class="card" style="text-align:center;padding:40px;color:var(--text3)">🗑️ Bin is empty</div>`
      :(!_binLsF.length
        ?`<div class="card" style="text-align:center;padding:34px;color:var(--text3)">No binned loadsheets match your search.</div>`
        :(search?'<div style="font-size:11px;color:var(--text3);margin:-4px 0 8px">'+_binLsF.length+' of '+binCount+' shown</div>':'')+
        (_binLsF.map(s=>{
  ...
  if(tab==='archive'){
    var _arch=search?archiveItems.filter(function(s){return _lsSearchMatch(s,search);}):archiveItems;
    return tabBar+(archiveItems.length?_savedSearchBox('Search archive — PIC, passenger, date, time, aircraft…'):'')+`
    ${!archiveItems.length
      ?'<div class="card" style="text-align:center;padding:40px;color:var(--text3)">📦 No archived loadsheets yet</div>'
      :(!_arch.length
        ?'<div class="card" style="text-align:center;padding:34px;color:var(--text3)">No archived loadsheets match your search.</div>'
        :(search?'<div style="font-size:11px;color:var(--text3);margin:-4px 0 8px">'+_arch.length+' of '+archiveItems.length+' shown</div>':'')+_arch.map(function(s){return _lsCard(s,{showUploaded:true,showUploadBtn:true});}).join(''))}`;
  }

  // (Saved manifests retired v28.59 — Operations is the Rezdy seatmap/loadsheet flow.)
  if(tab==='manifests'){S.savedTab='active';return renderSaved();}
