...
window.loadFlightRecords=async function(){
  ...
  try{
    // Paginate (PostgREST caps a plain select at 1000) by a UNIQUE key (id) so page boundaries are
    // STABLE — the legacy import gives every row the same updated_at, so paging on a non-unique column
    // skips/duplicates rows. By DEFAULT load only the last 12 months (the 25k-row full history made the
    // Records page slow); 'Load full history' (S._frLoadAll) lifts the window when older data is needed.
    var _dd=new Date(),_cut=(_dd.getFullYear()-1)+'-'+String(_dd.getMonth()+1).padStart(2,'0')+'-'+String(_dd.getDate()).padStart(2,'0');
    var _filt=S._frLoadAll?'':('&fr_date=gte.'+_cut);
    var rows=[],_pg=0,_PAGE=1000;
    while(_pg<80){
      var _rr=await _sbFetch(SB+'/rest/v1/ts_flight_records?select=*'+_filt+'&order=id.asc&limit='+_PAGE+'&offset='+(_pg*_PAGE),{headers:{...SH}});
...
// Full history is NEVER loaded automatically (25k+ rows = slow) — only on this explicit request, e.g.
// when an admin needs older data for a report. Reverts to the fast 12-month window with frLoad12mo.
window.frLoadAll=function(){if(S._frLoadAll)return;S._frLoadAll=true;S._frLoaded=true;if(typeof toast==='function')toast('Loading full history… this can take a moment.','info');if(window.loadFlightRecords)window.loadFlightRecords();if(typeof render==='function')render();};
window.frLoad12mo=function(){S._frLoadAll=false;S._frLoaded=true;if(window.loadFlightRecords)window.loadFlightRecords();if(typeof render==='function')render();};
...
function _frRenderTodayRecord(uid){
  ...
  // Live in-progress flights — off blocks set, not yet landed (done=false). Off-blocks persists to the
  // server immediately and flight records sync live, so this shows aircraft that are airborne right now.
  var live=[];Object.keys(S._frData||{}).forEach(function(k){var r=S._frData[k];if(r&&r.fr_date===date&&r.off&&!r.on&&!r.done&&(!selAc||r.aircraft===selAc))live.push(r);});
  if(live.length&&isToday){
    h+='<div class="card" style="border-left:4px solid #2563eb"><div style="font-size:12px;font-weight:800;color:#2563eb;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px">✈ In flight now</div>';
    live.sort(function(a,b){return (_frMins(a.off)||0)-(_frMins(b.off)||0);}).forEach(function(r){
      var ff=(S._ffData||{})[r.aircraft]||{};var eta=(ff.eta_type==='other'&&ff.eta_time)?('ETA '+ff.eta_time):'ETA via AMS';
      h+='<div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-top:1px solid var(--border2)"><span style="width:11px;height:11px;border-radius:50%;background:'+_frAcCol(r.aircraft)+';flex-shrink:0"></span><div style="flex:1;min-width:0"><div style="font-weight:800">'+_frEsc(_frAcShort(r.aircraft))+' <span style="font-weight:600;color:var(--text2)">'+_frEsc((r.from||'QN')+' → '+(r.to||'?'))+'</span></div><div style="font-size:12px;color:var(--text3)">Off '+_frEsc(r.off)+' · POB '+(+r.pob||0)+' · '+_frEsc(eta)+'</div></div><span style="flex-shrink:0;padding:3px 10px;border-radius:20px;background:rgba(37,99,235,.14);color:#2563eb;font-size:11px;font-weight:800">AIRBORNE</span></div>';
    });
    h+='</div>';
  }
...
function _frRenderBrowse(){
  ...
  var h='<div class="card">'+
    '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px"><div class="st" style="margin:0">Records — browse &amp; edit</div>'+
      ...
      (S._frLoadAll
        ? '<button onclick="window.frLoad12mo()" title="Back to the fast last-12-months view" style="padding:6px 12px;border-radius:8px;border:1px solid rgba(124,58,237,.5);background:rgba(124,58,237,.1);color:#a78bfa;font-size:12px;font-weight:700;cursor:pointer">✓ Full history loaded — back to 12 months</button>'
        : '<button onclick="window.frLoadAll()" title="Load every record incl. legacy history (slower) — for reports" style="padding:6px 12px;border-radius:8px;border:1px solid var(--border2);background:transparent;color:var(--text2);font-size:12px;font-weight:700;cursor:pointer">⤓ Load full history</button>')+
      '<button onclick="window.frbReset()" style="padding:6px 12px;border-radius:8px;border:1px solid var(--border2);background:transparent;color:var(--text2);font-size:12px;font-weight:700;cursor:pointer">↺ Clear filters</button></div></div>'+
    '<div style="font-size:11px;color:var(--text3);margin:4px 0 10px">Edit any cell to correct the database — each change saves immediately (aircraft TTIS totals are not auto-recalculated). '+(S._frLoadAll?'<b style="color:#a78bfa">Showing full history.</b>':'<b>Showing the last 12 months</b> — use “Load full history” for older records.')+'</div>'+
  ...
  all.forEach(function(r){
    var key;
    if(brk==='aircraft')key=_frAcShort(r.aircraft)||'—';
    else if(brk==='location'){var _lc=String(r.to||'').trim();key=_lc?((typeof _frPlaceName==='function'&&_frPlaceName(_lc))||_lc):'—';}   // landings count at the DESTINATION
    else key=_frPilotLabel(r.user_id);
    var e=bmap[key]||(bmap[key]={n:0,h:0,ldg:0});e.n++;e.h+=(+r.flightTime||0);e.ldg+=(+r.landings||0);
  });
  var brows=Object.keys(bmap).map(function(k){return {k:k,n:bmap[k].n,h:bmap[k].h,ldg:bmap[k].ldg};})
    .sort(function(a,b){return brk==='location'?(b.ldg-a.ldg)||(b.h-a.h):(b.h-a.h);});
  h+='<div style="margin-top:10px;display:flex;gap:6px;align-items:center;flex-wrap:wrap"><span style="font-size:11px;color:var(--text3);margin-right:2px">Breakdown:</span>'+
     '<button onclick="window.frbBreak(\'pilot\')" style="'+_frbChip(brk==='pilot')+'">By pilot</button>'+
     '<button onclick="window.frbBreak(\'aircraft\')" style="'+_frbChip(brk==='aircraft')+'">By aircraft</button>'+
     '<button onclick="window.frbBreak(\'location\')" style="'+_frbChip(brk==='location')+'">🛬 Landings by location</button></div>'+
     '<div style="overflow-x:auto;margin-top:6px"><table style="border-collapse:collapse;font-size:12px"><tbody>'+
     brows.slice(0,40).map(function(e){return '<tr><td style="padding:3px 16px 3px 0;font-weight:700">'+esc(e.k)+'</td><td style="padding:3px 16px 3px 0;text-align:right;color:var(--text3)">'+e.n+' flt</td><td style="padding:3px 16px 3px 0;text-align:right;color:var(--text2);font-weight:700">'+e.ldg+' ldg</td><td style="padding:3px 0;text-align:right;font-weight:800">'+e.h.toFixed(1)+'h</td></tr>';}).join('')+
     '</tbody></table></div></div>';
...
function renderLogbook(){
  ...
  h+='<div class="card"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px"><div><div style="font-size:17px;font-weight:800">'+_title+'</div>'+
    '<div style="font-size:12px;color:var(--text3);margin-top:2px">'+rows.length+' flights · total '+total.toFixed(1)+'h <span style="color:var(--text2)">(C208B '+ttC.toFixed(1)+'h · GA8 '+ttG.toFixed(1)+'h)</span>'+(S._frLoadAll?'':' · <span style="color:#a78bfa">last 12 months</span>')+'</div></div>'+
    '<div style="display:flex;gap:8px;flex-wrap:wrap">'+
    (S._frLoadAll
      ? '<button onclick="window.frLoad12mo()" title="Back to the fast last-12-months view" style="padding:9px 14px;border-radius:10px;border:1px solid rgba(124,58,237,.5);background:rgba(124,58,237,.1);color:#a78bfa;font-size:13px;font-weight:700;cursor:pointer">✓ Full history · back to 12 mo</button>'
      : '<button onclick="window.frLoadAll()" title="Load this pilot\'s full history (slower) — for a report" style="padding:9px 14px;border-radius:10px;border:1px solid var(--border2);background:transparent;color:var(--text2);font-size:13px;font-weight:700;cursor:pointer">⤓ Full history</button>')+
  ...
  days.forEach(function(d){
    ...
    if(_torder.length){
      ...
      var _o={},_oo=[];legs.forEach(function(r){var rt=_frRoute(r);if(_oo.indexOf(rt)<0)_oo.push(rt);_o[rt]=(_o[rt]||0)+1;});
      summary=_oo.map(function(rt){return _frEsc(rt)+(_o[rt]>1?' ×'+_o[rt]:'');}).join(' · ');
    }
