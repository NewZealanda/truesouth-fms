...
// ── days off (from the roster) ──
// A day counts as "free of duty" if the roster status is not a working code. Returns counts for the
// rule floors (2 off / any 14 days; 2 CONSECUTIVE off / any 30 days), looking back from asOf.
function _fdRosterStatus(uid,ds){var r=S.roster||{};var day=r[ds];return day?(day[uid]||''):'';}
function _fdIsOffDay(uid,ds){var st=_fdRosterStatus(uid,ds);if(!st)return false;var raw=st.indexOf('other:')===0?'other':st;return typeof ROSTER_WORKING!=='undefined'?!ROSTER_WORKING.has(raw):/rdo|off|leave|ul|sick/.test(raw);}
...
function _fdRenderRecord(canManage){
  ...
  h+='<div style="display:flex;gap:14px;flex-wrap:wrap;margin-top:8px;font-size:12px;color:var(--text2)">'+
    ...
    '<span style="color:var(--text3)">days off read from the roster (RDO/leave)</span>'+
    '</div></div>';
  ...
  h+='<div class="card" style="padding:0;overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12px;min-width:560px">';
  h+='<thead><tr style="background:var(--card2)">'+
    '<th style="text-align:left;padding:7px 8px;font-size:10px;color:var(--text3)">Date</th>'+
    '<th style="padding:7px 6px;font-size:10px;color:var(--text3)">Duty start</th>'+
    '<th style="padding:7px 6px;font-size:10px;color:var(--text3)">Duty end</th>'+
    '<th style="padding:7px 6px;font-size:10px;color:var(--text3)">Duty h</th>'+
    '<th style="padding:7px 6px;font-size:10px;color:var(--text3)">Flight h</th>'+
    (types.indexOf('c208b')>=0?'<th style="padding:7px 6px;font-size:10px;color:var(--text3)">C208B ldg</th>':'')+
    (types.indexOf('ga8')>=0?'<th style="padding:7px 6px;font-size:10px;color:var(--text3)">GA8 ldg</th>':'')+
    '<th style="padding:7px 6px;font-size:10px;color:var(--text3)" title="Disrupted-schedule extension to 12h">12h</th>'+
    '</tr></thead><tbody>';
  ...
  for(var dd=1;dd<=daysIn;dd++){
    ...
    var dHotCol=dh>L.duty_daily.val+1e-9?'#ef4444':(r.ext&&dh>11?'#f59e0b':'var(--text1)');
    var dow=_fdDateObj(ds).toLocaleDateString('en-NZ',{weekday:'short'});
    var isToday=ds===today;var off=_fdIsOffDay(uid,ds);
    h+='<tr style="border-top:1px solid var(--border2);'+(isToday?'background:rgba(124,58,237,.06)':off?'background:rgba(100,116,139,.05)':'')+'">'+
      ...
      '<td style="padding:3px 4px"><input type="time" value="'+_rzEscSafe(r.ds||'')+'" onchange="window.fdEditField(\''+uid+'\',\''+ds+'\',\'ds\',this.value)" style="width:88px;font-size:12px;padding:3px;border:1px solid var(--border2);border-radius:5px;background:var(--card);color:var(--text)"></td>'+
      '<td style="padding:3px 4px"><input type="time" value="'+_rzEscSafe(r.de||'')+'" onchange="window.fdEditField(\''+uid+'\',\''+ds+'\',\'de\',this.value)" style="width:88px;font-size:12px;padding:3px;border:1px solid var(--border2);border-radius:5px;background:var(--card);color:var(--text)"></td>'+
      '<td style="padding:3px 4px;text-align:center;font-weight:700;color:'+dHotCol+'">'+(dh?Math.round(dh*10)/10:'')+'</td>'+
      '<td style="padding:3px 4px"><input type="number" step="0.1" min="0" value="'+(r.ft!=null&&r.ft!==''?r.ft:'')+'" onchange="window.fdEditField(\''+uid+'\',\''+ds+'\',\'ft\',this.value)" style="width:56px;font-size:12px;padding:3px;border:1px solid var(--border2);border-radius:5px;background:var(--card);color:var(--text);text-align:center"></td>'+
      (types.indexOf('c208b')>=0?'<td style="padding:3px 4px"><input type="number" min="0" value="'+(r.lc!=null&&r.lc!==''?r.lc:'')+'" onchange="window.fdEditField(\''+uid+'\',\''+ds+'\',\'lc\',this.value)" style="width:46px;font-size:12px;padding:3px;border:1px solid var(--border2);border-radius:5px;background:var(--card);color:var(--text);text-align:center"></td>':'')+
      (types.indexOf('ga8')>=0?'<td style="padding:3px 4px"><input type="number" min="0" value="'+(r.lg!=null&&r.lg!==''?r.lg:'')+'" onchange="window.fdEditField(\''+uid+'\',\''+ds+'\',\'lg\',this.value)" style="width:46px;font-size:12px;padding:3px;border:1px solid var(--border2);border-radius:5px;background:var(--card);color:var(--text);text-align:center"></td>':'')+
      '<td style="padding:3px 4px;text-align:center"><input type="checkbox" '+(r.ext?'checked':'')+' onchange="window.fdToggle(\''+uid+'\',\''+ds+'\',\'ext\')"></td>'+
      '</tr>';
  }
  h+='</tbody></table></div>';
  h+='<div style="font-size:11px;color:var(--text3);padding:6px 2px">Advisory only — your printed &amp; signed monthly record is the controlled document. Limits per AC119-2 / Exposition; tune them in Settings ▸ Operations ▸ Flight &amp; Duty.</div>';
  return h;
}
