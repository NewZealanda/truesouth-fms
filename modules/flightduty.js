...
// ── Certify with a drawn signature (mirrors the loadsheet sign box) ──
// Tap "Certify & sign" → opens a modal with a signature canvas; once drawn, the signature is saved on
// the cert and embedded in the printout.
window.fdCertifyOpen=function(uid,month){if(!uid||!month)return;S._fdSign={uid:uid,month:month,sig:''};render();};
window.fdSignCancel=function(){S._fdSign=null;render();};
window.fdSignClear=function(){var c=document.getElementById('fdSigCanvas');if(c)c.getContext('2d').clearRect(0,0,c.width,c.height);if(S._fdSign)S._fdSign.sig='';render();};
window.fdSignConfirm=function(){
  if(!S._fdSign)return;
  if(!S._fdSign.sig){if(typeof toast==='function')toast('Please sign in the box first.','warn');return;}
  window.fdCertify(S._fdSign.uid,S._fdSign.month,S._fdSign.sig);
};
window.fdCertify=async function(uid,month,sig){
  if(!uid||!month)return;
  S._fdCerts=S._fdCerts||{};var k=uid+'|'+month;
  S._fdCerts[k]={at:new Date().toISOString(),by:(S.user&&S.user.name)||'',sig:sig||''};
  S._fdSign=null;render();
  if(typeof sbU==='function'){
    var row={id:k,user_id:uid,period:month,certified_at:S._fdCerts[k].at,certified_by:S._fdCerts[k].by};
    if(sig)row.signature=sig;
    var ok=await sbU('ts_fd_certs',[row]);
    // The `signature` column may not be applied yet — if the write was rejected, record the cert
...
function _fdSetupSig(){
  var c=document.getElementById('fdSigCanvas');if(!c||c._fdWired)return;c._fdWired=true;var ctx=c.getContext('2d');
  if(S._fdSign&&S._fdSign.sig){var img=new Image();img.onload=function(){ctx.drawImage(img,0,0,c.width,c.height);};img.src=S._fdSign.sig;}
  var drawing=false;
  ...
  c.onmousemove=c.ontouchmove=function(e){if(!drawing)return;e.preventDefault();var p=pos(e);ctx.lineWidth=2.5;ctx.lineCap='round';ctx.strokeStyle='#1e293b';ctx.lineTo(p.x,p.y);ctx.stroke();ctx.beginPath();ctx.moveTo(p.x,p.y);};
  c.onmouseup=c.onmouseleave=c.ontouchend=function(){if(drawing){drawing=false;var had=!!(S._fdSign&&S._fdSign.sig);if(S._fdSign)S._fdSign.sig=c.toDataURL('image/png');if(!had)render();}};
}
function _fdSignModal(){
  if(!S._fdSign)return '';
  var u=(S.users||[]).find(function(x){return x.id===S._fdSign.uid;});var nm=(u&&u.name)||'';
  return '<div onclick="window.fdSignCancel()" style="position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9100;display:flex;align-items:center;justify-content:center;padding:16px">'+
    '<div onclick="event.stopPropagation()" style="background:var(--card);border:1px solid var(--border);border-radius:14px;max-width:480px;width:100%;padding:18px;box-shadow:0 20px 60px rgba(0,0,0,.5)">'+
      '<div style="font-size:15px;font-weight:800;color:var(--text1);margin-bottom:2px">Certify '+_rzEscSafe(_fdMonthLabel(S._fdSign.month))+'</div>'+
      '<div style="font-size:12px;color:var(--text3);margin-bottom:10px">'+_rzEscSafe(nm)+' — sign below to certify this month’s flight &amp; duty record as correct and complete.</div>'+
      '<div style="position:relative;border:1px solid var(--border2);border-radius:8px;background:#fff;overflow:hidden;touch-action:none">'+
        '<canvas id="fdSigCanvas" width="600" height="150" style="width:100%;height:150px;display:block;cursor:crosshair"></canvas>'+
        (!S._fdSign.sig?'<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;pointer-events:none;color:#94a3b8;font-size:14px;letter-spacing:.1em">DRAW SIGNATURE HERE</div>':'')+
      '</div>'+
      '<div style="display:flex;gap:8px;margin-top:12px;justify-content:flex-end;flex-wrap:wrap">'+
        '<button onclick="window.fdSignClear()" style="padding:8px 14px;border-radius:8px;border:1px solid var(--border2);background:transparent;color:var(--text2);font-size:13px;font-weight:700;cursor:pointer">Clear</button>'+
        '<button onclick="window.fdSignCancel()" style="padding:8px 14px;border-radius:8px;border:1px solid var(--border2);background:transparent;color:var(--text3);font-size:13px;cursor:pointer">Cancel</button>'+
        '<button onclick="window.fdSignConfirm()" style="padding:8px 16px;border-radius:8px;border:none;background:#22c55e;color:#fff;font-size:13px;font-weight:800;cursor:pointer">✓ Certify &amp; sign</button>'+
      '</div>'+
    '</div></div>';
}
...
window.loadFlightDuty=async function(){
  ...
  try{
    ...
    if(Array.isArray(certs)){var cc={};certs.forEach(function(r){cc[r.user_id+'|'+r.period]={at:r.certified_at,by:r.certified_by||'',sig:r.signature||''};});S._fdCerts=cc;}
  ...
  if(cert){
    h+='<div class="card" style="border-left:3px solid #22c55e;display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">'+
      '<div style="font-size:12px;color:var(--text2);display:flex;align-items:center;gap:10px">'+
        (cert.sig?'<img src="'+cert.sig+'" alt="signature" style="height:34px;width:auto;background:#fff;border:1px solid var(--border2);border-radius:4px;padding:1px 3px">':'')+
        '<span>✓ '+_rzEscSafe(_fdMonthLabel(month))+' certified by '+_rzEscSafe(cert.by)+(cert.at?' on '+_rzEscSafe(_fdFmt(String(cert.at).slice(0,10))):'')+'.</span></div>'+
    ...
    h+='<div class="card" style="border-left:3px solid '+(monthEnded?'#f59e0b':'var(--border2)')+';display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">'+
      ...
      '<div style="display:flex;gap:6px;flex-shrink:0">'+
        '<button class="btn btn-ghost" style="font-size:12px" onclick="window.fdPrint(\''+uid+'\',\''+month+'\')">🖨 Print</button>'+
        '<button class="btn btn-ghost" style="font-size:12px;border-color:rgba(74,222,128,.5);color:#4ade80" onclick="window.fdCertifyOpen(\''+uid+'\',\''+month+'\')">✓ Certify &amp; sign</button>'+
      '</div></div>';
  }
  return h;
}
...
window.fdPrint=function(uid,month){
  ...
  w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Flight & Duty — '+_rzEscSafe(nm)+' — '+_fdMonthLabel(month)+'</title><style>'+
    ...
    '<div class="sign">'+(cert?('<div>'+(cert.sig?'<img src="'+cert.sig+'" style="max-height:64px;width:auto;display:block;margin-bottom:3px">':'')+'Certified by '+_rzEscSafe(cert.by)+(cert.at?' on '+_rzEscSafe(_fdFmt(String(cert.at).slice(0,10))):'')+'</div>'):'<div>Pilot signature &amp; date</div>')+'</div>'+
