...
function _rzSmartSeat(acId,paxList,opts){
  ...
  var groups=Object.keys(gMap).map(function(k){var g=gMap[k];g.w=g.pax.reduce(function(s,p){return s+wOf(p);},0);g.size=g.pax.length;g.inf=g.pax.some(function(p){return p.infant;});g._placed=false;return g;});
  ...
  var pairRows=(frontSingle!=null?rows.slice(1):rows).map(function(r){return r.slice();}); // remaining rows, front→back
  var result={};
  // Take `need` seats from consecutive rows (front, or back for infant groups). Removes them from pairRows.
  function takeRows(need,fromBack){
    var order=[];for(var i=0;i<pairRows.length;i++)order.push(i);if(fromBack)order.reverse();
    for(var oi=0;oi<order.length;oi++){
      var start=order[oi],got=[],used=[],r=start;
      while(r>=0&&r<pairRows.length&&got.length<need){
        if(used.length&&r!==used[used.length-1]+1)break;
        if(pairRows[r].length===0){if(got.length)break;else{r++;continue;}}
        for(var s=0;s<pairRows[r].length&&got.length<need;s++)got.push(pairRows[r][s]);
        used.push(r);r++;
      }
      if(got.length>=need){got=got.slice(0,need);got.forEach(_rmSeat);return got;}
    }
    var allFree=[];pairRows.forEach(function(r){r.forEach(function(s){allFree.push(s);});});
    if(allFree.length>=need){var pick=allFree.slice(0,need);pick.forEach(_rmSeat);return pick;}
    return null;
  }
  function _rmSeat(seat){for(var rr=0;rr<pairRows.length;rr++){var ix=pairRows[rr].indexOf(seat);if(ix>=0){pairRows[rr].splice(ix,1);return;}}}
  // Seat one group across its seats: heaviest member most-forward, infant host rearmost.
  function assignGroup(g,seats){
    var seatsByArm=seats.slice().sort(function(x,y){return arm(x)-arm(y);}); // front→back
    var nonInf=g.pax.filter(function(p){return !p.infant;}).sort(function(x,y){return wOf(y)-wOf(x);});
    var inf=g.pax.filter(function(p){return p.infant;});
    var mem=nonInf.concat(inf); // infants last → rear seats
    seatsByArm.forEach(function(seat,i){if(mem[i])result[seat]=mem[i].id;});
    g._placed=true;
  }
  // 1) Front single → heaviest solo, else one member of the heaviest odd (non-infant) group.
  if(frontSingle!=null){
    var solos=groups.filter(function(g){return g.size===1&&!g.inf;}).sort(function(x,y){return y.w-x.w;});
    var fg=solos[0];
    if(!fg){var odds=groups.filter(function(g){return g.size%2===1&&!g.inf;}).sort(function(x,y){return y.w-x.w;});fg=odds[0];}
    if(fg){
      if(fg.size===1){result[frontSingle]=fg.pax[0].id;fg._placed=true;}
      else{var extra=takeRows(fg.size-1,false);if(extra){assignGroup(fg,[frontSingle].concat(extra));}else{result[frontSingle]=fg.pax[0].id;fg._placed=true;}}
      frontSingle=null;
    }
  }
  // 2) Remaining groups: non-infant heaviest first (front pairs), infant groups to the back.
  var rest=groups.filter(function(g){return !g._placed;});
  var ordered=rest.filter(function(g){return !g.inf;}).sort(function(x,y){return y.w-x.w;}).concat(rest.filter(function(g){return g.inf;}).sort(function(x,y){return y.w-x.w;}));
  ordered.forEach(function(g){
    var seats=takeRows(g.size,g.inf);
    if(!seats){ // last resort: any free seat, incl. an unused front single
      seats=[];if(frontSingle!=null){seats.push(frontSingle);frontSingle=null;}
      var pool=[];pairRows.forEach(function(r){r.forEach(function(s){pool.push(s);});});
      while(seats.length<g.size&&pool.length){var s=pool.shift();seats.push(s);_rmSeat(s);}
    }
    if(seats&&seats.length)assignGroup(g,seats);
  });
  return result;
}
