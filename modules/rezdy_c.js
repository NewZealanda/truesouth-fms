    ...
    if(_grp){
      ...
      detailH='<div class="card" style="border-left:4px solid '+_gcol+'">'+
        '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-bottom:6px">'+
          ...
          '</div>'+
          (function(){var _dt=String(_grp.key||'').split('|')[1]||_grp.start;var _dep=(typeof _schedDepManualLock==='function')&&_schedDepManualLock(S.rezdyDate,_dt);return '<button class="btn btn-ghost" style="font-size:12px;color:'+(_dep?'#22c55e':'var(--text2)')+';border-color:'+(_dep?'rgba(34,197,94,.5)':'var(--border2)')+'" title="'+(_dep?'Departed — every booking is locked to its current aircraft, so the calendar keeps showing it here after it leaves and across a refresh. Click to undo.':'Mark this departure DEPARTED — locks each booking to the aircraft it left on so it stays here after departure + on refresh (use this instead of waiting for the time to pass).')+'" onclick="event.stopPropagation();window.schedToggleLock(\''+S.rezdyDate+'\',\''+_rzEsc(_dt).replace(/'/g,"\\'")+'\')">'+(_dep?'✓ Departed':'🛫 Mark departed')+'</button>';})()+
