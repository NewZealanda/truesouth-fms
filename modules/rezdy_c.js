...
function _rzSearchSeat(dep,acId,paxList,coPic){
  ...
  var rows=rowPairsForAc(acId,coPic);if(!rows.length)return null;
  var arm=function(i){return (a.seats[i]&&a.seats[i].arm)||0;};
  var wOf=function(p){var w=parseFloat(p&&p.weight);return isNaN(w)?0:w;};
  ...
  var best=null,bestD=Infinity,bestBack=null,bestBackD=Infinity,count=0,CAP=2200,abort=false;
  var infIds={};paxList.forEach(function(p){if(p.infant)infIds[p.id]=1;});
  function consider(order){
    ...
    if(sc.dist<bestD){bestD=sc.dist;best=res;}
    if(order.length&&order[order.length-1].inf&&sc.dist<bestBackD){bestBackD=sc.dist;bestBack=res;} // infant group at the back
  }
  ...
  return bestBack||best;   // prefer infant-at-back within limits; else any within-limits
}
...
window.rezdyManAllocate=function(){
  ...
  // 2) Build booking groups from the rest (solos = size-1 groups).
  var gMap={};unpinned.forEach(function(p){var k=(p.group&&String(p.group).trim())||('solo:'+p.id);(gMap[k]||(gMap[k]={pax:[]})).pax.push(p);});
  var groups=Object.keys(gMap).map(function(k){var g=gMap[k];g.size=g.pax.length;g.w=g.pax.reduce(function(s,p){return s+(parseFloat(p.weight)||0);},0);return g;});
  ...
  groups.forEach(function(g){
    ...
    //    so a bigger group can still claim a bigger gap). Keeps the group together.
    var fit=fleet.filter(function(id){return free(id)>=members.length;}).sort(function(a,b){return free(a)-free(b);})[0];
    ...
    while(members.length){
      var best=fleet.filter(function(id){return free(id)>0;}).sort(function(a,b){return free(b)-free(a);})[0];
