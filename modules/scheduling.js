  ...
  (function(){
    ...
    if(!dp.departures.length){
      ...
      dp.departures.forEach(function(d){
        ...
        h+='<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:12px;color:var(--text2);margin-bottom:4px">'+
          '<b style="color:var(--text1);min-width:46px">'+_schedEsc(d.time)+'</b>'+
          '<span style="color:var(--text3)">'+d.pax+' pax →</span>'+acs+
          (d.short?'<span style="color:#ef4444;font-weight:700;font-size:11px">⚠ short '+(d.pax-d.cap)+'</span>':'')+'</div>';
      });
...
function _schedDayPlan(date){
  ...
  var used={},plan=[],cost=0,loadedLegs=0,emptyLegs=0;
  deps.forEach(function(d){
    var fleet=_schedFleetFor(date,d.time);
    var cand=fleet.map(function(f){var ll=_schedLoadedLeg(f.ac),el=_schedEmptyLeg(f.ac);var reused=!!used[f.ac];
      return {ac:f.ac,cap:f.cap,priority:f.priority,airvan:f.airvan,reused:reused,ll:ll,el:el,inc:2*ll+(reused?2*el:0)};});
    var chosen=_schedPlanPick(d.pax,cand);
    var depAc=[];var depCap=0;
    chosen.forEach(function(c){used[c.ac]=true;depCap+=c.cap;depAc.push({ac:c.ac,cap:c.cap,reused:c.reused});
      loadedLegs+=2;cost+=2*c.ll;if(c.reused){emptyLegs+=2;cost+=2*c.el;}});
    plan.push({time:d.time,pax:d.pax,aircraft:depAc,cap:depCap,short:depCap<d.pax});
  });
  var acUsed=Object.keys(used);
