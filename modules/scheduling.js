...
function _schedLocIdx(locs,dest,fb){
  ...
  var al=SCHED_DEST_ALIAS[dest]||[];
  for(var j=0;j<locs.length;j++){var nm=String((locs[j]&&locs[j].name)||'').toLowerCase();for(var k=0;k<al.length;k++){if(nm.indexOf(al[k])>=0)return j;}}
  return (fb==null?-1:fb);
}
...
function _schedRouteCompute(dest,typeKey,leg){
  ...
  if(rc&&typeof _bizFlightCost==='function'&&typeof _bizState==='function'){
    try{var p=_bizState();var locs=p.running.locations||[];
      var depIdx=_schedLocIdx(locs,'QN',0);var destIdx=_schedLocIdx(locs,(dest||'MF'),-1);
...
function renderSchedulingSettings(){
  ...
  // Allocation order — the exact ranking the auto-allocator adheres to, highest priority first. This is
  // what the engine does: seat capacity is a hard filter; among everything that fits, it minimises the
  // whole day's cost (planned across ALL unlocked departures before the day starts); aircraft priority
  // is only a tiebreak, so it's the first soft rule broken when a cheaper plan needs it; and a manual
  ...
  var _aoRules=[
    ...
    ['Caravans on Milford / airvans elsewhere','Keeps the caravans (SLA/SLB/SDB) on the Milford run so big-plane seats stay open for last-minute bookings, and sends the airvans to Mt Cook / Franz / other. Cost-BOUNDED: it only overrides the cheapest plan when doing so adds no more than the $ threshold below per mis-matched aircraft. Set the threshold to $0 to turn this off.','#a78bfa'],
  ...
  h+='<div class="card" style="margin-bottom:14px"><div class="st">Allocation order</div>'+
    '<p style="font-size:12px;color:var(--text3);margin:-4px 0 10px">What the plane/passenger allocator adheres to, highest first. Locked / departed departures are never re-planned.</p>';
  _aoRules.forEach(function(r,i){
    h+='<div style="display:flex;align-items:flex-start;gap:10px;padding:8px 0'+(i<_aoRules.length-1?';border-bottom:1px solid var(--border2)':'')+'">'+
      '<span style="flex-shrink:0;width:22px;height:22px;border-radius:50%;background:'+r[2]+'22;border:1px solid '+r[2]+';color:'+r[2]+';font-size:12px;font-weight:800;display:inline-flex;align-items:center;justify-content:center">'+(i+1)+'</span>'+
      '<div><div style="font-size:13px;font-weight:800;color:var(--text1)">'+_schedEsc(r[0])+(i===0?' <span style="font-size:10px;color:#ef4444;font-weight:800">HARD</span>':(i===2?' <span style="font-size:10px;color:#f59e0b;font-weight:800">FIRST TO BREAK</span>':''))+'</div>'+
  ...
  h+='<div style="margin-top:10px;padding:8px 10px;border-radius:9px;background:rgba(96,165,250,.10);border:1px solid rgba(96,165,250,.4);font-size:11.5px;color:var(--text2);line-height:1.45">'+
    ...
    '<div style="margin-top:10px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding-top:10px;border-top:1px solid var(--border2)">'+
      '<span style="font-size:12.5px;font-weight:800;color:#a78bfa">Caravan/Milford preference threshold</span>'+
      '<span style="font-size:11.5px;color:var(--text3)">override the cheapest plan to honour the rule only when it adds ≤</span>'+
      '$<input type="number" min="0" step="10" value="'+_schedEsc(String(_schedRoutePrefW()))+'" onchange="schedSetRoutePrefW(this.value)" style="width:74px;padding:5px 7px;border-radius:7px;border:1px solid var(--border2);background:var(--bg2);color:var(--text1);font-size:12.5px;font-weight:700;text-align:right">'+
      '<span style="font-size:11.5px;color:var(--text3)">per mis-matched aircraft per day (0 = off)</span>'+
    '</div>'+
  '</div>';
...
function renderResources(){
  ...
  (function(){
    ...
    if(!dp.departures.length){
      ...
      // When the chosen plan ferries a tail, cost the alternative that uses only FRESH aircraft (the
      // "spread") so the operator can SEE why ferrying won — ferry $X vs spread $Y on the live costs.
      var _nf=(dp.emptyLegs>0)?_schedSimulate(sel,{noReuse:true}):null;
      ...
      if(_nf){
        h+='<div style="font-size:11px;color:var(--text3);margin-top:4px;display:flex;gap:14px;flex-wrap:wrap;align-items:center">'+
          (_nf.paxShort<=dp.paxShort
            ? 'Chosen <b style="color:#22c55e">'+_schedMoney(dp.cost)+'</b> (with ferry) vs spread/no-ferry <b style="color:var(--text2)">'+_schedMoney(_nf.cost)+'</b> · ferrying '+(_nf.cost>dp.cost?'saves <b style="color:#22c55e">'+_schedMoney(_nf.cost-dp.cost)+'</b>':'costs <b style="color:#ef4444">'+_schedMoney(dp.cost-_nf.cost)+'</b> more')
            : 'A no-ferry "spread" can\'t carry everyone today (would leave <b style="color:#ef4444">'+(_nf.paxShort-dp.paxShort)+'</b> short), so ferrying is required.')+
          '</div>';
      }
      if(ci.callIn){
...
// CONSTRAINT RANKING (highest first) — the single source of truth for "what the allocator adheres to":
//   1. HARD  seat capacity: a subset only qualifies if total seats ≥ pax AND every booking group packs
//            whole (_schedCanPack). Never violated.
//   2. SOFT  lowest cost: `inc` (route + empty-ferry $) minimised first, then fewer ferries, then fewer
//            aircraft — i.e. the cheapest plan. _schedDayPlan applies this across the WHOLE day.
//   3. SOFT  aircraft priority (★): `-pri` is a low tiebreak, so it's the FIRST rule broken — a cheaper
//            non-priority aircraft wins over a priority one. (User/manual pins win above all of this via
//            _rzBookingAc; locked/departed departures are skipped entirely by _schedDayPlan.)
// The covering score is [inc, fer, cnt, -pri, cap] in that exact precedence.
...
// The forward, resource-aware simulation for one day. `forced` (optional) maps a departure KEY → an
// array of aircraft ids to USE for that UNLOCKED departure instead of the cost-greedy pick. With no
// forced map this is EXACTLY the original greedy. The whole-day refinement (in _schedDayPlan) feeds
// candidate assignments through here and re-costs the entire day, so the day total it returns already
// includes every loaded leg + every empty ferry the assignment implies.
function _schedSimulate(date,opts,forced){
  ...
  var _prefW=_schedRoutePrefW();   // route-preference weight ($/mismatched aircraft) — objective only, not $ cost
  var freeAt={},freeAtDest={},everUsed={},plan=[],cost=0,mis=0,loadedLegs=0,emptyLegs=0,paxShort=0,usedAtTime={};
  ...
  deps.forEach(function(d){
    ...
    if(locked){
      ...
      fixed.forEach(function(a){var ll=_schedLoadedLeg(a.ac,d.dest),el=_schedEmptyLeg(a.ac,d.dest);var out=(freeAt[a.ac]!=null&&freeAt[a.ac]>dm);
        var cap=_schedNum(_schedTail(a.ac).cap)||_schedDefaultCap(a.ac);ldCap+=cap;everUsed[a.ac]=true;freeAt[a.ac]=dm+_schedDepDurMin(d);freeAtDest[a.ac]=d.dest;_ut[a.ac]=1;mis+=_schedAcRouteMismatch(a.ac,d.dest);_mxUse(a.ac,_schedDepFltHrs(d.dests,_schedIsAirvan(a.ac)));
    ...
    var cand=fleet.map(function(f){var ll=_schedLoadedLeg(f.ac,d.dest),el=_schedEmptyLeg(f.ac,d.dest);
      ...
      var mm=_schedAcRouteMismatch(f.ac,d.dest);   // route-preference penalty (objective only; NOT added to cost below)
      return {ac:f.ac,cap:f.cap,priority:f.priority,airvan:f.airvan,reused:out,isNew:!everUsed[f.ac],ll:ll,el:el,mm:mm,inc:2*ll+(out?2*el:0)+_prefW*mm};})
      .filter(function(c){return !(c.reused&&(_schedNoFerry(c.ac)||opts.noReuse));});   // no-ferry aircraft can't reposition; opts.noReuse = a fresh-tails-only "spread" comparison plan
    var chosen;var fset=forced[d.key];
    if(fset&&fset.length){
      // Forced assignment from the refinement — use only the forced tails still VALID this departure
      // (serviceable, free at this time, within maintenance hours). Any that aren't drop out, which
      // lowers capacity → paxShort rises → the refinement rejects the move. Same cand objects, so the
      // reuse/ferry cost is computed identically to the greedy.
      var _byAc={};cand.forEach(function(c){_byAc[c.ac]=c;});
      chosen=fset.map(function(ac){return _byAc[ac];}).filter(Boolean);
    } else {
      chosen=_schedPlanPick(d.pax,cand,maxNew,d.groups);
    }
    var depAc=[];var depCap=0;
    chosen.forEach(function(c){everUsed[c.ac]=true;freeAt[c.ac]=dm+_schedDepDurMin(d);freeAtDest[c.ac]=d.dest;_ut[c.ac]=1;mis+=(c.mm||0);_mxUse(c.ac,_schedDepFltHrs(d.dests,c.airvan));depCap+=c.cap;depAc.push({ac:c.ac,cap:c.cap,reused:c.reused});
  ...
  return {date:date,departures:plan,cost:Math.round(cost),mis:mis,prefW:_prefW,loadedLegs:loadedLegs,emptyLegs:emptyLegs,paxShort:paxShort,aircraftUsed:acUsed,pilotsNeeded:acUsed.length};
}
// Feasible aircraft SETS that could cover one departure (seats ≥ pax AND every booking group packs
// whole), cheapest static route cost first. The refinement re-costs each through _schedSimulate so the
// real winner accounts for ferries too. Capped so it's cheap; only single-time aircraft are eligible.
function _schedDepCandidateSets(date,d){
  var fleet=_schedFleetFor(date,d.time);var n=fleet.length;if(!n)return [];
  if(n>14){fleet=fleet.slice(0,14);n=14;}
  var groups=d.groups||[];var out=[];
  for(var mask=1;mask<(1<<n);mask++){
    var caps=[],acs=[],cap=0,cost=0,cnt=0;
    for(var i=0;i<n;i++)if(mask&(1<<i)){var f=fleet[i];acs.push(f.ac);caps.push(f.cap);cap+=f.cap;cost+=f.cost;cnt++;}
    if(cnt>3)continue;                         // a single departure realistically never needs >3 tails
    if(cap<d.pax)continue;
    if(typeof _schedCanPack==='function'&&!_schedCanPack(caps,groups))continue;
...
// and keep a move only if the verified day TOTAL is strictly cheaper (and pax-short no worse). Because
// every kept move lowers the total, the result is never worse than the greedy — it just catches cases
// the greedy misses, e.g. committing a cheap Airvan at 0930 that then forces a pricey empty ferry at
// 1200 when SDB-at-0930 + a caravan-at-1200 is cheaper overall. Locked/departed departures are never
// touched (their aircraft stay exactly as flown). To skip refinement pass opts.maxAircraft (the cap
// sweep) or opts.noRefine.
function _schedDayPlan(date,opts){
  ...
  for(var round=0;round<8;round++){
    ...
    function _consider(assign){
      ...
      if(sim.paxShort<=best.paxShort&&_obj(sim)<_obj(best)-0.5&&(!move||_obj(sim)<_obj(move.plan)))move={assign:assign,plan:sim};
    }
