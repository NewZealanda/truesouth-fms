...
window.rezdySetDate=function(v){
  // Calendar day-step: snapshot the WHOLE on-screen calendar body (header cards + grid / pilot &
  // aircraft-movement columns) FIRST, so the loading render shows the old day dimmed in place
  // (no "Loading…" flash, page height preserved) — see _rzCalLoadingHold.
  try{var _cb=(S.section==='operations'&&S.tab==='calendar')?document.getElementById('rzCalBody'):null;S._calSnap=(_cb&&_cb.innerHTML)?_cb.innerHTML:null;}catch(e){S._calSnap=null;}
