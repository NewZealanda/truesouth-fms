...
function initRealtime(){
  ...
  const tables=['ts_crew','ts_aircraft','ts_users','ts_loadsheets','ts_manifests','ts_charter_rates','ts_settings','ts_maintenance','ts_flight_records','ts_flightduty','ts_fd_certs','ts_maint_forms','ts_vehicle_prestarts','ts_ops_notices','ts_ops_notice_reads','ts_equipment','ts_visitors','ts_flight_following','ts_wx_links','ts_native_bookings'].filter(function(t){return !(_hashFree()&&t==='ts_users');});
  try{
          ...
          if(msg.payload&&msg.payload.status==='ok'){
            ...
            if(_rtConnectedOnce){try{
              ...
              if(S.rezdyDate){
                ...
                if(Array.isArray(S._rezdyBookings)&&typeof window.platformReloadBookingsLive==='function')window.platformReloadBookingsLive();
              }
...
async function reloadTable(table){
  if(table==='ts_loadsheets'){
    ...
    return false;
  } else if(table==='ts_native_bookings'){
    // Native (in-house) bookings — a create/edit/delete on another device: re-pull + merge live.
    if(Array.isArray(S._rezdyBookings)&&typeof window.platformReloadBookingsLive==='function')window.platformReloadBookingsLive();
...
document.addEventListener('visibilitychange',function(){
  if(document.hidden){
    ...
    // Mobile sleep PAUSES the setTimeout that refreshes the JWT, so on return the token can be expired
    // and the next save (e.g. signing a loadsheet) 401s and is queued. Proactively refresh the session
    // when we come back if it's expired/near-expiry, then flush any queued writes so a signed loadsheet
    // /flight record that 401'd while the phone was asleep uploads as soon as we're back.
    if(S.user){try{
      var _near=(typeof AUTH_PHASE_C!=='undefined'&&AUTH_PHASE_C&&_sbSession&&_sbSession.expires_at&&(_sbSession.expires_at-Date.now())<180000);
      if(_near&&typeof _sbRefresh==='function'){_sbRefresh().then(function(){if(typeof _syncFlush==='function')_syncFlush();},function(){});}
      else if(typeof _syncFlush==='function'){_syncFlush();}
    }catch(e){}}
...
window.updateLoginSuffix=function(){
  ...
  var pad=1;                                   // input has no internal padding → just 1px for the caret, so the @suffix TOUCHES the last letter and moves with it as you type
  var avail=(e.parentElement?e.parentElement.clientWidth:320)-(s?s.offsetWidth:200)-6;
  e.style.flex='0 0 auto';e.style.width=Math.max(8,Math.min(m.offsetWidth+pad,avail))+'px';
};
function logout(){
  if(AUTH_PHASE_C){
