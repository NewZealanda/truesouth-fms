...
function _bizState(){
  ...
  if(!S._bizPlan.paxData||typeof S._bizPlan.paxData!=='object')S._bizPlan.paxData=(typeof _bizPaxDefault==='function')?_bizPaxDefault():{};
...
function renderBusinessPlan(){
  ...
  var body=(tab==='mainplan')?_bizRenderMainPlan():(tab==='loans')?_bizRenderLoans():(tab==='running')?_bizRenderRunning()
    :(tab==='timeline')?_bizRenderTimeline():(tab==='roster')?_bizRenderRoster():(tab==='pax')?_bizRenderPax():(tab==='budget')?_bizRenderBudget():(tab==='pnl')?_bizRenderPnl():_bizComingSoon(tabs.find(function(x){return x.id===tab;}));
...
function _bizRenderPaxDaily(){
  ...
  var _inS='width:54px;font-size:12px;padding:3px 3px;border:1px solid var(--border2);border-radius:5px;background:var(--card);color:var(--text);text-align:center;box-sizing:border-box';
  ...
  // table
  var col='min-width:54px';
  h+='<div class="card" style="padding:0;overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12px;min-width:480px;table-layout:fixed">';
  h+='<colgroup><col style="width:74px"><col style="width:58px"><col style="width:54px"><col style="width:62px"><col style="width:58px"><col style="width:62px"><col style="width:64px"></colgroup>';
  ...
  for(var d=1;d<=daysIn;d++){
    ...
    h+='<tr style="border-top:1px solid var(--border2)">'+
      '<td style="padding:2px 4px;white-space:nowrap;font-size:11px"><b>'+d+'</b> <span style="color:var(--text3)">'+dow+'</span></td>'+
      '<td style="padding:2px 2px"><input type="number" value="'+(row.p26!=null?row.p26:'')+'" onchange="window.bizSetPax(\''+ds+'\',\'p26\',this.value)" style="'+_inS+'"></td>'+
      '<td style="padding:2px 4px;text-align:center;font-weight:700">'+(accum||'')+'</td>'+
      '<td style="padding:2px 4px;text-align:center;font-weight:700'+(ht?';background:'+ht.bg+';color:'+ht.fg:';color:var(--text3)')+'">'+(tracking!=null?(tracking>=0?'+':'')+Math.round(tracking*100)+'%':'—')+'</td>'+
      '<td style="padding:2px 2px"><input type="number" value="'+(row.p25!=null?row.p25:'')+'" onchange="window.bizSetPax(\''+ds+'\',\'p25\',this.value)" style="'+_inS+'"></td>'+
      '<td style="padding:2px 4px;text-align:center;color:var(--text3)">'+(runLy||'')+'</td>'+
      '<td style="padding:2px 4px;text-align:center;font-weight:700'+(hc?';background:'+hc.bg+';color:'+hc.fg:';color:var(--text3)')+'">'+(compare!=null?Math.round(compare*100)+'%':'—')+'</td>'+
      '</tr>';
  }
  h+='</tbody></table></div>';
  // bottom figures
