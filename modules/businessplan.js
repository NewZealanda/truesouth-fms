...
function _bizState(){
  ...
  if(!S._bizPlan.paxSync||typeof S._bizPlan.paxSync!=='object')S._bizPlan.paxSync={}; // ym -> ISO time of last Rezdy sync (shared)
...
window.bizSetFuel=function(k,value){var p=_bizState();p.running.fuel[k]=(value===''?'':(+value));p.running.fuelUpdated=new Date().toISOString();_bizSave();if(typeof safeRender==='function')safeRender();};
...
// Refresh EVERY day of a month from Rezdy into ts_rezdy_bookings (the cache is filled per-day, so
// days never opened in Bookings would otherwise be missing). Limited concurrency to be gentle.
async function _bizSyncMonthRezdy(ym){
  if(typeof callFn!=='function')return;
  var y=+ym.slice(0,4),mo=+ym.slice(5,7),daysIn=new Date(y,mo,0).getDate();
  var days=[];for(var d=1;d<=daysIn;d++)days.push(ym+'-'+String(d).padStart(2,'0'));
  var idx=0;
  async function worker(){while(idx<days.length){var ds=days[idx++];try{await callFn('rezdy-sync',{date:ds});}catch(e){}}}
  var ws=[];for(var k=0;k<4;k++)ws.push(worker());
...
window.bizPaxSyncRezdy=async function(ym,auto){
  ...
  try{
    // Current/future months: pull the WHOLE month live from Rezdy first (so un-viewed days are
    // included). Past months use whatever is already cached.
    if(ym>=cur)await _bizSyncMonthRezdy(ym);
    var rows=await sbF('ts_rezdy_bookings','&tour_date=gte.'+from+'&tour_date=lte.'+to);
    if(!Array.isArray(rows)){if(!auto&&typeof toast==='function')toast('No bookings found for that month.','warn');return;}
    ...
    _bizSave();render();
    if(typeof toast==='function')toast(auto?('Pulled provisional bookings for '+ym+' ✓'):('Updated '+n+' day'+(n===1?'':'s')+' from Rezdy seats sold ✓'),'ok');
  }catch(e){if(!auto&&typeof toast==='function')toast('Could not pull from Rezdy — '+e,'err');}
};
...
// Diverging heat scale with DARK extremes. [lo,hi] is the normal band: t=0 red (bad) → t=0.5 amber
// → t=1 green (good). Beyond that it keeps ramping so "really good / really bad" reads at a glance:
// good goes green → blue → purple → black (≈ +50% over the band lands near black), and bad darkens
// red → black. The fill also gets stronger toward the extremes, with white text once it's dark.
function _bizHeat(val,lo,hi){
  var t=(val-lo)/(hi-lo);t=Math.max(-1,Math.min(2.5,t));
  var stops=[[-1,[22,22,26]],[0,[239,68,68]],[0.5,[245,158,11]],[1,[34,197,94]],[1.5,[37,99,235]],[2,[139,92,246]],[2.5,[17,17,20]]];
  var r,g,b;
  for(var i=0;i<stops.length-1;i++){
    var c0=stops[i],c1=stops[i+1];
    if(t<=c1[0]||i===stops.length-2){var u=Math.max(0,Math.min(1,(t-c0[0])/(c1[0]-c0[0])));
      r=Math.round(c0[1][0]+(c1[1][0]-c0[1][0])*u);g=Math.round(c0[1][1]+(c1[1][1]-c0[1][1])*u);b=Math.round(c0[1][2]+(c1[1][2]-c0[1][2])*u);break;}
  }
  var ext=(t<0)?(-t):Math.max(0,(t-1)/1.5);      // 0 across the normal red→green band, →1 at the dark ends
  var alpha=0.20+0.60*Math.min(1,ext);
  var fg=(alpha>=0.5)?'#ffffff':'rgb('+Math.round(r*0.85)+','+Math.round(g*0.7)+','+Math.round(b*0.7)+')';
  return {bg:'rgba('+r+','+g+','+b+','+alpha.toFixed(2)+')',fg:fg};
}
// Original "minor" scale — clamped red → amber → green (no dark extremes). Used for the Compare column.
function _bizHeatMinor(val,lo,hi){
...
function _bizRenderPaxDaily(){
  ...
  // Provisional numbers are pulled on demand via the Sync button (saved to the shared plan, so every
  // viewer sees the same data + when it was last refreshed).
  var _syncIso=(p.paxSync||{})[ym];
  function _bizFmtSync(iso){var d=iso?new Date(iso):null;if(!d||isNaN(d))return '';return d.toLocaleString('en-NZ',{weekday:'long',day:'numeric',month:'short',hour:'numeric',minute:'2-digit'});}
  var _syncNote=_syncIso?'<span style="font-size:11px;color:var(--text3)">Last updated '+_rzEscSafe(_bizFmtSync(_syncIso))+'</span>':'';
  var h='<div class="card" style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">'+
    ...
    '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">'+_syncNote+
  ...
  for(var d=1;d<=daysIn;d++){
    ...
    var ht=tracking!=null?_bizHeat(tracking,-0.15,0.15):null;            // Tracking → extended (black) scale
    var hc=compare!=null?_bizHeatMinor(compare,0.7,1.3):null;            // Compare → original minor scale
...
function _bizRenderRunning(){
  ...
  var bar='<div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap">'+
    tabs.map(function(t){return '<button class="sub-tab '+(tab===t.id?'on':'')+'" onclick="window.bizRunTab(\''+t.id+'\')">'+t.lbl+'</button>';}).join('')+'</div>';
  var body=(tab==='maint')?_bizRunMaint():(tab==='vars')?_bizRunVars():_bizRunCalcView();
  return '<div>'+bar+body+'</div>';
}

// ── Calculator (main): aircraft × flight time + landings/airways from a Dep→Dest ──
function _bizRunCalcView(){
    ...
    '</div></div>';
  {
    ...
    h+='</div></div>';
    // ── Total flight cost — running cost + the per-place charges ──
    h+='<div class="card"><div class="st">Total flight cost — '+(r.hrs||0)+' hr · lands '+(r.stops.length?r.stops.map(function(s){return _rzEscSafe(s.name);}).join(' + '):'—')+'</div><div style="font-size:13px">';
    h+=row('Running cost ('+(r.hrs||0)+' hr)',_bizPh(r.flight));
    r.stops.forEach(function(s){h+=row('Landing — '+_rzEscSafe(s.name),_bizPh(s.ldg));if(s.conc)h+=row('Concession — '+_rzEscSafe(s.name),_bizPh(s.conc));if(s.aw)h+=row('Airways — '+_rzEscSafe(s.name),_bizPh(s.aw));});
    h+=row('Total (ex GST)',_bizPh(r.total),true);
...
// ── Maintenance: per-aircraft maintenance → $/hr ──
function _bizRunMaint(){
  var p=_bizState();var fuel=p.running.fuel;
...
// ── Cost variables: fuel (price + per-aircraft burn) and landings/airways per location ──
function _bizRunVars(){
  var p=_bizState();var fuel=p.running.fuel;
  var _fU=p.running.fuelUpdated?('<span style="font-size:11px;color:var(--text3);font-weight:400">Last updated '+_rzEscSafe(_bizFmtWhen(p.running.fuelUpdated))+'</span>'):'';
  var h='<div class="card"><div class="st" style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap"><span>Fuel <span style="font-weight:400;font-size:11px;color:var(--text3)">— ex GST</span></span>'+_fU+'</div>'+
    '<div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;margin-bottom:10px">'+
      '<div><label style="font-size:10px;color:var(--text3);display:block;margin-bottom:2px">AvGas $/L</label><input type="number" step="0.01" value="'+(fuel.avgas!==''?fuel.avgas:'')+'" onchange="window.bizSetFuel(\'avgas\',this.value)" style="'+_bizInS+';width:90px;text-align:right"></div>'+
      '<div><label style="font-size:10px;color:var(--text3);display:block;margin-bottom:2px">Jet A1 $/L</label><input type="number" step="0.01" value="'+(fuel.jeta1!==''?fuel.jeta1:'')+'" onchange="window.bizSetFuel(\'jeta1\',this.value)" style="'+_bizInS+';width:90px;text-align:right"></div>'+
    '</div>'+
    '<div style="overflow-x:auto"><table style="border-collapse:collapse;font-size:11.5px;min-width:420px">'+
    '<thead><tr style="background:var(--card2)">'+['AC','Burn L/hr','Fuel','Fuel/hr'].map(function(t,i){return '<th style="text-align:'+(i<1?'left':'center')+';padding:6px 8px;font-size:9px;color:var(--text3);font-weight:700">'+t+'</th>';}).join('')+'</tr></thead><tbody>';
  p.running.aircraft.forEach(function(ac,i){var c=_bizRunCalc(ac,fuel);
    h+='<tr style="border-top:1px solid var(--border2)">'+
      '<td style="padding:2px 8px;font-weight:700">'+_rzEscSafe(ac.code||'')+'</td>'+
      '<td style="padding:2px 4px"><input type="number" step="1" value="'+(ac.burn!=null&&ac.burn!==''?ac.burn:'')+'" onchange="window.bizSetRunAc('+i+',\'burn\',this.value)" style="'+_bizInS+';width:64px;text-align:right"></td>'+
      '<td style="padding:2px 4px"><select onchange="window.bizSetRunAc('+i+',\'fuelType\',this.value)" style="'+_bizInS+';width:74px"><option value="jeta1"'+(ac.fuelType==='jeta1'?' selected':'')+'>Jet A1</option><option value="avgas"'+(ac.fuelType==='avgas'?' selected':'')+'>AvGas</option></select></td>'+
      '<td style="padding:2px 8px;text-align:right;font-weight:700;color:var(--text2)">'+_bizPh(c.fuelHr)+'</td>'+
      '</tr>';
  });
  h+='</tbody></table></div></div>';
