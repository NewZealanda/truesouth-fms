...
function _bizRenderPaxDaily(){
  ...
  // A future month holds only provisional forward bookings (no confirmed history yet) — auto-pull
  // them from Rezdy the first time it's opened this session, so the numbers are there without a click.
  if(ym>curMonth){S._bizPaxAutoSynced=S._bizPaxAutoSynced||{};if(!S._bizPaxAutoSynced[ym]){S._bizPaxAutoSynced[ym]=1;if(typeof window.bizPaxSyncRezdy==='function')setTimeout(function(){window.bizPaxSyncRezdy(ym,true);},0);}}
