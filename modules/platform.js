...
// ── Platform config (ts_settings key 'platform_cfg') ────────────────────────
// Currently: the recurring WINTER timetable window (MM-DD .. MM-DD, every year).
var _PF_CFG_DEFAULT={winterFrom:'05-01',winterTo:'09-30'};
window.platformCfg=function(){var c=S._pfCfg;return (c&&typeof c==='object')?Object.assign({},_PF_CFG_DEFAULT,c):Object.assign({},_PF_CFG_DEFAULT);};
window.platformCfgLoad=function(){
  if(S._pfCfgLoaded)return;S._pfCfgLoaded=true;
  try{var cch=lsGet('ts_platform_cfg');if(cch&&typeof cch==='object')S._pfCfg=cch;}catch(e){}
  try{fetch(SB+'/rest/v1/ts_settings?key=eq.platform_cfg&select=value',{headers:SH})
    .then(function(r){return r.ok?r.json():[];})
    .then(function(rows){var v=rows&&rows[0]&&rows[0].value;if(typeof v==='string'){try{v=JSON.parse(v);}catch(e){v=null;}}
      if(v&&typeof v==='object'){S._pfCfg=v;try{lsSet('ts_platform_cfg',v);}catch(e){}render();}}).catch(function(){});
  }catch(e){}
};
window.platformCfgSet=function(field,val){
  var v=String(val||'').trim();
  if(!/^\d{2}-\d{2}$/.test(v)){if(typeof toast==='function')toast('Use MM-DD, e.g. 05-01','warn');render();return;}
  S._pfCfg=Object.assign(platformCfg(),{});S._pfCfg[field]=v;
  try{lsSet('ts_platform_cfg',S._pfCfg);}catch(e){}
  if(typeof sbMergeSave==='function')sbMergeSave('platform_cfg',S._pfCfg,function(m){S._pfCfg=m;try{lsSet('ts_platform_cfg',m);}catch(e){}});
  render();
};
// Is a YYYY-MM-DD flight date inside the recurring winter window?
window.platformIsWinter=function(date){
  var m=/^\d{4}-(\d{2}-\d{2})$/.exec(String(date||'').slice(0,10));if(!m)return false;
  var c=platformCfg(),d=m[1];
  return (c.winterFrom<=c.winterTo)?(d>=c.winterFrom&&d<=c.winterTo):(d>=c.winterFrom||d<=c.winterTo); // window may wrap year-end
};
// Departure times for a product on a FLIGHT date: winter timetable when the date
// falls in the winter window AND the product has winter times; else the default.
window.platformTimesFor=function(p,date){
  if(!p)return [];
  var w=(p.times_winter||[]);
  return (platformIsWinter(date)&&w.length)?w:(p.times||[]);
};
...
// field = 'times' (summer/default) or 'times_winter' (winter timetable).
window.platformProdTimes=function(code,val,field){
  var p=platformProduct(code);if(!p)return;
  field=(field==='times_winter')?'times_winter':'times';
  p[field]=String(val||'').split(',').map(function(s){
...
function renderAdminProducts(){
  ...
  var h='<div class="card"><div class="st" style="margin-bottom:6px">Products &amp; Pricing</div>'+
    '<p style="font-size:12px;color:var(--text3);margin:-2px 0 8px;line-height:1.5">The commercial catalog behind the booking platform: customer-facing name, per-passenger prices (NZD incl. GST), standard departure times and public visibility. '+
    ...
    '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 12px;font-size:12px;color:var(--text2)">'+
      '<b>❄ Winter timetable window (every year):</b>'+
      '<input id="pf_wfrom" type="text" value="'+esc(cfg.winterFrom)+'" placeholder="05-01" onchange="window.platformCfgSet(\'winterFrom\',this.value)" style="'+ist+';width:64px" title="MM-DD">'+
      '<span>to</span>'+
      '<input id="pf_wto" type="text" value="'+esc(cfg.winterTo)+'" placeholder="09-30" onchange="window.platformCfgSet(\'winterTo\',this.value)" style="'+ist+';width:64px" title="MM-DD">'+
      '<span style="color:var(--text3)">MM-DD · flights inside this window use each product’s ❄ Winter times (blank = same as summer).</span></div>';
  if(!list.length){
    ...
    list.forEach(function(p){
      ...
      h+='<tr style="border-top:1px solid var(--border2)">'+
        ...
        '<td style="padding:4px 6px"><input id="pr_'+cj+'_tm" type="text" value="'+esc((p.times||[]).join(', '))+'" placeholder="08:00, 09:30" onchange="window.platformProdTimes(\''+cj+'\',this.value,\'times\')" style="'+ist+';width:130px"></td>'+
        '<td style="padding:4px 6px"><input id="pr_'+cj+'_tw" type="text" value="'+esc((p.times_winter||[]).join(', '))+'" placeholder="same as ☀" onchange="window.platformProdTimes(\''+cj+'\',this.value,\'times_winter\')" style="'+ist+';width:130px"></td>'+
      ...
      h+='<tr><td></td><td colspan="15" style="padding:0 6px 6px"><input id="pr_'+cj+'_ds" type="text" value="'+esc(p.description||'')+'" placeholder="Short public description (optional)" onchange="window.platformProdSet(\''+cj+'\',\'description\',this.value)" style="'+ist+';width:100%;font-size:11px;color:var(--text2)"></td></tr>';
    });
    h+='</table></div>';
  }
  var d=S._prodDraft||{};
