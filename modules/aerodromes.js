...
function renderAerodromes(){
  ...
  var rows=filt.map(function(a){
    ...
    return '<tr data-icao="'+a.icao+'" onclick="window.aeroSelect(this.dataset.icao)" style="cursor:pointer;background:'+(isSelected?'rgba(99,102,241,.12)':'')+';">'
      ...
      +'<td style="padding:5px 8px;font-size:12px;color:var(--text1)">'+a.name+(isCustom?' <span style="font-size:9px;color:var(--ok-text);border:1px solid var(--ok-text);border-radius:3px;padding:1px 3px">edited</span>':'')+'</td>'
  ...
  var selInfo=selEntry
    ?'<div style="padding:6px 10px;background:rgba(99,102,241,.1);border:1px solid rgba(99,102,241,.3);border-radius:6px;font-size:12px;color:var(--text2);margin-bottom:8px"><span style="font-weight:700;color:var(--acc)">'+selEntry.icao+'</span> &#x2014; '+selEntry.name+' &middot; Elev '+selEntry.elev+'ft &middot; '+selEntry.lat.toFixed(4)+', '+selEntry.lon.toFixed(4)+'</div>'
