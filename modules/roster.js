...
function renderRosterView(){
  ...
  // ── View mode: day | week (default) | month — per-device, persisted. ──
  var view=S._rosterView||(S._rosterView=lsGet('ts_roster_view')||'week');
  if(['day','week','month'].indexOf(view)<0)view='week';
  var _anchor=S.rosterWeek?new Date(S.rosterWeek+'T00:00:00'):new Date(today);
  if(isNaN(_anchor.getTime()))_anchor=new Date(today);
  var days=[],prevStart,nextStart,wkLbl;
  if(view==='day'){
    var dd=new Date(_anchor);S.rosterWeek=_rIso(dd);
    days=[dd];
    prevStart=new Date(dd);prevStart.setDate(prevStart.getDate()-1);
    nextStart=new Date(dd);nextStart.setDate(nextStart.getDate()+1);
    var _DFULL=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
    wkLbl=_DFULL[dd.getDay()]+' '+dd.getDate()+' '+MONTHS[dd.getMonth()]+' '+dd.getFullYear();
  } else if(view==='month'){
    var m0=new Date(_anchor.getFullYear(),_anchor.getMonth(),1);S.rosterWeek=_rIso(m0);
    var _mn=new Date(m0.getFullYear(),m0.getMonth()+1,0).getDate();
    for(var _mi=0;_mi<_mn;_mi++)days.push(new Date(m0.getFullYear(),m0.getMonth(),1+_mi));
    prevStart=new Date(m0.getFullYear(),m0.getMonth()-1,1);
    nextStart=new Date(m0.getFullYear(),m0.getMonth()+1,1);
    var _MFULL=['January','February','March','April','May','June','July','August','September','October','November','December'];
    wkLbl=_MFULL[m0.getMonth()]+' '+m0.getFullYear();
  } else {
    // Week: aligned to the active mode — Mon–Sun normally, Thu–Wed for pay week.
    ...
    nextStart=new Date(weekStart);nextStart.setDate(nextStart.getDate()+7);
  ...
  var _dayWide=(view==='day');   // day view: the single status widget fills the column (esp. mobile)
  ...
  h+='<div style="display:inline-flex;border:1px solid var(--border2);border-radius:8px;overflow:hidden">';
  [['day','Day'],['week','Week'],['month','Month']].forEach(function(vv){
    var on=view===vv[0];
    h+='<button tabindex="-1" onclick="window.rosterSetView(\''+vv[0]+'\')" style="padding:5px 11px;border:none;background:'+(on?'rgba(167,139,250,.18)':'var(--card2)')+';color:'+(on?'#a78bfa':'var(--text3)')+';font-size:11px;font-weight:700;cursor:pointer">'+vv[1]+'</button>';
  });
  h+='</div>';
  if(_payAllowed&&view==='week'){
  ...
  h+='<th style="padding:8px '+(S.mobileView?'6px':'12px')+';text-align:left;font-size:11px;font-weight:700;color:var(--text3);min-width:'+(S.mobileView?'46px':'155px')+(S.mobileView?';width:46px':(_dayWide?';width:155px':''))+';position:sticky;left:0;z-index:21;background:var(--card2);border-right:1px solid var(--border)">Crew</th>';
  days.forEach(function(d,i){
    var ds=_rIso(d),isTdy=ds===todayStr,isWe=d.getDay()===0||d.getDay()===6;
    h+='<th style="padding:5px 3px;text-align:center;min-width:'+cellW+'px;background:'+(isTdy?'rgba(124,58,237,.14)':'var(--card2)')+';border-bottom:1px solid var(--border)">';
  ...
  if(!displayUsers.length){
    h+='<tr><td colspan="'+_colspan+'" style="padding:32px;text-align:center;color:var(--text3);font-size:13px">No users found.</td></tr>';
  } else {
    var grouped=[];
    ROLE_GROUPS.forEach(function(g){
    ...
    grouped.forEach(function(grp){
      h+='<tr style="background:var(--card2)">';
      h+='<td colspan="'+_colspan+'" style="padding:4px 12px;font-size:10px;font-weight:800;color:'+grp.g.col+';text-transform:uppercase;letter-spacing:.08em;position:sticky;left:0">'+grp.g.label+'</td>';
      h+='</tr>';
      ...
      grp.members.forEach(function(u){
        ...
        // Sticky name cell — on mobile just the 2-letter initials (tap/hover shows the full name).
        h+='<td style="padding:5px '+(S.mobileView?'6px':'10px')+';white-space:nowrap;position:sticky;left:0;z-index:1;background:var(--card);border-right:1px solid var(--border)">';
        if(S.mobileView){
          h+='<span title="'+esc(u.name)+'" style="font-size:12px;font-weight:800;padding:2px 6px;border-radius:5px;background:'+rowCol+'1a;color:'+rowCol+'">'+ini+'</span>';
        } else {
        ...
        days.forEach(function(d,i){
          var ds=_rIso(d);
          var isTdy=ds===todayStr,isWe=d.getDay()===0||d.getDay()===6;
          var st=_rGetStatus(u,ds,roster);
          ...
          if(isHidden){h+='<span style="display:inline-block;width:'+cellW+'px;height:24px"></span>';}else if(canEdit){
            ...
            h+='<select tabindex="-1" onchange="window.rosterSetCell(\''+u.id+'\',\''+ini+'\',\''+ds+'\',this.value)" title="'+(isOther?esc(otherNote):'')+'" style="padding:'+(_dayWide?'8px 6px':'4px '+(_cmp?'0':'3px'))+';border-radius:6px;border:1px solid '+(st?cfg.bd:'var(--border)')+';background:'+(st?cfg.bg:'transparent')+';color:'+(st?cfg.col:'var(--text3)')+';font-size:'+(_cmp?'9px':(_dayWide?'13px':'11px'))+';font-weight:700;cursor:pointer;width:'+(_dayWide?'94%':cellW+'px')+(_dayWide?';max-width:300px':'')+';text-align:center">';
            ...
            h+='<span'+(isOther?' title="'+esc(otherNote)+'"':'')+' style="display:inline-block;padding:'+(_dayWide?'8px 6px':'4px '+(_cmp?'1px':'3px'))+';border-radius:6px;border:1px solid '+(st?cfg.bd:'var(--border)')+';background:'+(st?cfg.bg:'transparent')+';color:'+(st?cfg.col:'var(--text3)')+';font-size:'+(_cmp?'9px':(_dayWide?'13px':'11px'))+';font-weight:700;'+(_dayWide?'width:94%;max-width:300px;box-sizing:border-box':'min-width:'+(cellW-10)+'px;max-width:'+cellW+'px')+';overflow:hidden;text-overflow:ellipsis;vertical-align:middle">'+dispLbl+'</span>';
          }
          h+='</td>';
        });
        h+='</tr>';
...
window.loadRosterLeave=async function(){
  ...
  endD.setDate(endD.getDate()+Math.max(31,(S._rosterRangeDays||7))-1);   // always ≥31 days so day/week/month views are all covered (loader can run before the view computes its range)
...
// Switch the roster view mode (day | week | month). Persisted per device.
window.rosterSetView=function(v){
  if(['day','week','month'].indexOf(v)<0)v='week';
  S._rosterView=v;try{lsSet('ts_roster_view',v);}catch(e){}
  S._rosterLeaveWeek=null;render();
};
// Shift the displayed roster period by ±1 (arrow keys), mirroring the ◁ ▷ buttons — a day, week
// or month depending on the active view. Routes through _navAway so an unsaved draft prompts first.
window.rosterShiftWeek=function(delta){
  window._navAway(function(){
    ...
    var v=S._rosterView||'week';
    if(v==='day')base.setDate(base.getDate()+(delta||0));
    else if(v==='month')base.setMonth(base.getMonth()+(delta||0));
    else base.setDate(base.getDate()+(delta||0)*7);
...
window.rosterJump=function(v){
  ...
  window._navAway(function(){
    var vw=S._rosterView||'week';
    if(vw==='week'){var day=d.getDay();var diff=day===0?-6:1-day;d.setDate(d.getDate()+diff);}      // snap to Monday
    else if(vw==='month'){d=new Date(d.getFullYear(),d.getMonth(),1);}                               // snap to 1st
