...
function _mfWorkOrderHtml(f){
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Southern Lakes Aviation Form 13 - Work Order</title><style>'+
...
function renderMaintFormEditor(id){
  ...
  // Stable element ids so focus+caret survive a re-render (safeRender force-renders after 3s even while
  // an input is focused; without an id the global focus-restore can't recover it → "can't type"). v28.x
  var _fid=String(id).replace(/[^a-z0-9]/gi,'_');
  function _mfId(path){return 'mf_'+_fid+'_'+String(path).replace(/[^a-z0-9]/gi,'_');}
  function fld(label,path,val,ph){return '<div><label style="font-size:11px;color:var(--text3);font-weight:600">'+label+'</label><input id="'+_mfId(path)+'" value="'+_mfEsc(val)+'" placeholder="'+(ph||'')+'" oninput="window.mfField(\''+id+'\',\''+path+'\',this.value)" style="'+_in+'"></div>';}
  ...
  function ta(label,path,val){return '<div style="flex:1 1 100%"><label style="font-size:11px;color:var(--text3);font-weight:600">'+label+'</label><textarea id="'+_mfId(path)+'" oninput="window.mfField(\''+id+'\',\''+path+'\',this.value)" style="'+_ta+'">'+_mfEsc(val)+'</textarea></div>';}
  ...
  function section(no,label,key){
    ...
    var rowsH=items.map(function(it,i){
      ...
      return '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px">'+
        ...
        '<input id="'+_mfId(key+'_'+i+'_t')+'" value="'+_mfEsc(it.text)+'" oninput="window.mfItemText(\''+id+'\',\''+key+'\','+i+',this.value)" style="flex:1;min-width:0;padding:6px 8px;background:var(--card2);border:1px solid var(--border2);border-radius:6px;color:var(--text1);font-size:13px">'+
        ...
        '<input id="'+_mfId(key+'_'+i+'_i')+'" value="'+_mfEsc(it.initial)+'" placeholder="Init" oninput="window.mfItemInitial(\''+id+'\',\''+key+'\','+i+',this.value)" title="Engineer initial" style="width:48px;padding:6px 4px;text-align:center;background:var(--card2);border:1px solid var(--border2);border-radius:6px;color:var(--text1);font-size:12px">'+
  ...
  function oilSection(){
    ...
    return '<div style="border:1px solid var(--border2);border-radius:8px;padding:10px">'+
      ...
      '<input id="'+_mfId('oil.note')+'" value="'+_mfEsc(d.oil&&d.oil.note)+'" placeholder="Oil note (optional)" oninput="window.mfField(\''+id+'\',\'oil.note\',this.value)" style="'+_in+';margin-top:8px">'+
    '</div>';
  }
