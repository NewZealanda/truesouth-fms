...
function _wbEnvRows(a){var e=a&&a.cogEnv;if(!Array.isArray(e))return null;var r=e.filter(function(p){return p&&isFinite(+p.w)&&isFinite(+p.fwd)&&isFinite(+p.aft);}).map(function(p){return {w:+p.w,fwd:+p.fwd,aft:+p.aft};}).sort(function(x,y){return x.w-y.w;});return r.length>=2?r:null;}
