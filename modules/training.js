...
var _TRAIN_TIPS=[
  ...
  {icon:'🏠',title:'Set your start page + sound',body:'Tap your name (top-right) ▸ User settings & preferences. Pick which page the app opens to, switch light/dark, and choose your notification chime (tap ▶ to hear each) or turn notification sounds off.'},
  {icon:'🌤️',title:'Weather calls',body:'The Weather page (under Operations in the menu) lists every departure for the day. Any pilot can record a call — Called ON (green), Cancelled (red, with reason + next best day) or a Final call in the office (amber) — each with a comment. If no call is made, the PIC gets a reminder 15 min before the 1-hour-prior deadline, and the Today page shows each flight\'s status at a glance.'},
  {icon:'🚐',title:'Park all the vans',body:'On Transport you can now park EVERY vehicle for a departure (⊘ Park all) — the run just sits as a list waiting for a van/driver. ▶ Activate all puts them back. Drag a single van on/off a run as usual.'},
  {icon:'📋',title:'One loadsheet per flight',body:'Re-pushing the same flight (same aircraft, date and departure) to a loadsheet now overwrites its existing Active draft instead of stacking up copies. Want a separate version? Use Duplicate — and a signed loadsheet is never overwritten.'},
  {icon:'👨‍✈️',title:'Everything pilot in one place',body:'Flight Record, Logbooks and Flight & Duty all live together under Crew in the menu (along with Roster & Leave).'},
