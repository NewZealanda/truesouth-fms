  ...
  if(!flights.length){
    ...
    flights.forEach(function(f){
      ...
      h+='<div onclick="window.todayNav(\'calendar\')" style="cursor:pointer;display:flex;align-items:center;gap:10px;padding:8px 4px;border-bottom:1px solid var(--border2)">'
        ...
        +(function(){if(typeof _wxCall!=='function')return '';var c=_wxCall(f.hhmm);if(!c||!c.status)return '<span title="No weather call yet" style="font-size:11px;color:var(--text3);font-weight:700">⛅</span>';var col=(typeof _wxStatusColor==='function')?_wxStatusColor(c.status):'#94a3b8';var ic=c.status==='on'?'✓ Wx':c.status==='cancelled'?'✕ Wx':'⚠ Wx';return '<span title="'+_sodEsc((typeof _wxStatusLabel==='function'?_wxStatusLabel(c.status):c.status)+(c.reason?' — '+c.reason:''))+'" style="font-size:11px;font-weight:800;color:'+col+';white-space:nowrap">'+ic+'</span>';})()
