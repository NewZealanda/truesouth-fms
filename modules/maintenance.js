...
function renderMaintObservations(){
  ...
  // Split: open items, recently-sorted (lined out, kept 30 days), and older-sorted (behind a dropdown).
  var nowMs=Date.now(), CUT=30*86400000;
  function _resAt(e){var t=e.resolvedAt?Date.parse(e.resolvedAt):0;return isNaN(t)?0:t;}
  var open=all.filter(function(e){return e.status!=='resolved';});
  var resAll=all.filter(function(e){return e.status==='resolved';});
  var recentRes=resAll.filter(function(e){return (nowMs-_resAt(e))<CUT;});
  var oldRes=resAll.filter(function(e){return (nowMs-_resAt(e))>=CUT;});
  var openDefects=open.filter(function(e){return e.type==='defect';}).length;
  h+='<div style="font-size:12px;color:var(--text3);margin-bottom:8px">'+(openDefects?'<span style="color:#ef4444;font-weight:700">'+openDefects+' open defect'+(openDefects!==1?'s':'')+'</span>':'No open defects')+(open.length?' · '+open.length+' open':'')+'</div>';

  function obsCard(e){
    var col=TC[e.type]||'#94a3b8',resolved=e.status==='resolved';
    var d=e.ts?new Date(e.ts):null;
    var dstr=d?d.toLocaleDateString('en-NZ',{day:'numeric',month:'short',year:'numeric'}):'';
    var s='<div class="card" style="margin-bottom:8px;border-left:4px solid '+col+'">';
    s+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap">';
    s+='<span style="font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:'+col+';background:'+col+'1a;padding:2px 8px;border-radius:5px">'+(TL[e.type]||'Note')+'</span>';
    if(e.type==='defect'&&!resolved)s+='<span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:5px;background:rgba(239,68,68,.15);color:#ef4444">Open</span>';
    if(resolved)s+='<span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:5px;background:rgba(34,197,94,.15);color:#22c55e">✓ Sorted</span>';
    s+='<span style="font-size:11px;color:var(--text3);margin-left:auto">'+_obsEsc(e.author||'')+' · '+dstr+(e.editedAt?' · edited':'')+'</span></div>';
    var _editing=canEdit&&S._obsEditId===e.id;
    if(_editing){
      var _etype=S._obsEditType||e.type||'observation';
      s+='<div style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap">';
      [['observation','Observation'],['defect','Defect'],['note','Note']].forEach(function(t){var on=_etype===t[0];var tc=TC[t[0]]||'#94a3b8';s+='<button tabindex="-1" onclick="S._obsEditType=\''+t[0]+'\';render()" style="font-size:11px;padding:5px 12px;border-radius:14px;border:1px solid '+(on?tc:'var(--border2)')+';background:'+(on?tc+'1a':'transparent')+';color:'+(on?tc:'var(--text3)')+';cursor:pointer;font-weight:'+(on?'700':'500')+'">'+t[1]+'</button>';});
      s+='</div>';
      s+='<textarea id="obs-edit-text" oninput="S._obsEditText=this.value" rows="3" style="width:100%;padding:10px 12px;background:var(--card2);border:1px solid var(--border2);border-radius:8px;color:var(--text1);font-size:13px;box-sizing:border-box;resize:vertical;font-family:inherit">'+_obsEsc(S._obsEditText!=null?S._obsEditText:(e.text||''))+'</textarea>';
      s+='<div style="display:flex;gap:8px;justify-content:flex-end;margin-top:8px">';
      s+='<button tabindex="-1" onclick="window.maintObsEditCancel()" style="font-size:12px;padding:6px 14px;border-radius:7px;border:1px solid var(--border2);background:transparent;color:var(--text3);cursor:pointer">Cancel</button>';
      s+='<button tabindex="-1" onclick="window.maintObsEditSave(\''+e.id+'\')" style="font-size:12px;padding:6px 16px;border-radius:7px;border:none;background:var(--acc);color:#fff;font-weight:700;cursor:pointer">Save</button></div>';
    } else {
      s+='<div style="font-size:13px;color:var(--text1);white-space:pre-wrap;line-height:1.5'+(resolved?';text-decoration:line-through;color:var(--text3)':'')+'">'+_obsEsc(e.text||'')+'</div>';
      if(resolved)s+='<div style="font-size:11px;color:#22c55e;margin-top:4px">✓ Sorted'+(e.resolvedInitial?' ('+_obsEsc(e.resolvedInitial)+')':'')+(e.resolvedAt?' · '+new Date(e.resolvedAt).toLocaleDateString('en-NZ',{day:'numeric',month:'short',year:'numeric'}):'')+(e.resolvedVia?' · on a work order':'')+(e.resolvedNote?' — '+_obsEsc(e.resolvedNote):'')+'</div>';
      if(canEdit){
        if(!resolved){
          s+='<div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;align-items:center">';
          s+='<input id="obs-note-'+e.id+'" placeholder="Sign-off note (optional)" style="flex:1;min-width:150px;padding:7px 10px;background:var(--card2);border:1px solid var(--border2);border-radius:6px;color:var(--text1);font-size:12px">';
          s+='<button tabindex="-1" onclick="window.maintObsSort(\''+e.id+'\')" style="font-size:12px;padding:7px 14px;border-radius:6px;border:none;background:#22c55e;color:#fff;font-weight:800;cursor:pointer">✓ Sort</button>';
          s+='<button tabindex="-1" onclick="window.maintObsEditStart(\''+e.id+'\')" style="font-size:11px;padding:6px 10px;border-radius:6px;border:1px solid var(--border2);background:transparent;color:var(--acc);cursor:pointer">Edit</button>';
          s+='<button tabindex="-1" onclick="window.maintObsDelete(\''+e.id+'\')" style="font-size:11px;padding:6px 10px;border-radius:6px;border:1px solid rgba(239,68,68,.4);background:transparent;color:#ef4444;cursor:pointer">Delete</button></div>';
        } else {
          s+='<div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap">';
          s+='<button tabindex="-1" onclick="window.maintObsReopen(\''+e.id+'\')" style="font-size:11px;padding:4px 10px;border-radius:6px;border:1px solid var(--border2);background:transparent;color:var(--text3);cursor:pointer">Reopen</button>';
          s+='<button tabindex="-1" onclick="window.maintObsDelete(\''+e.id+'\')" style="font-size:11px;padding:4px 10px;border-radius:6px;border:1px solid rgba(239,68,68,.4);background:transparent;color:#ef4444;cursor:pointer">Delete</button></div>';
        }
      }
    }
    s+='</div>';return s;
  ...
  if(!open.length&&!recentRes.length&&!oldRes.length){
    h+='<div class="card" style="text-align:center;color:var(--text3);padding:30px">No observations logged for '+acDisp(sel)+' yet.</div>';
  } else {
    open.forEach(function(e){h+=obsCard(e);});
    if(recentRes.length){
      h+='<div style="font-size:11px;color:var(--text3);font-weight:700;margin:16px 0 6px;text-transform:uppercase;letter-spacing:.05em">Recently sorted</div>';
      recentRes.forEach(function(e){h+=obsCard(e);});
    }
    if(oldRes.length){
      var openOld=!!S._obsShowOld;
      h+='<button tabindex="-1" onclick="S._obsShowOld=!S._obsShowOld;render()" style="margin-top:14px;width:100%;text-align:left;font-size:12px;padding:8px 12px;border-radius:8px;border:1px solid var(--border2);background:transparent;color:var(--text3);cursor:pointer">'+(openOld?'▾':'▸')+' Older sorted ('+oldRes.length+')</button>';
      if(openOld){h+='<div style="margin-top:8px">';oldRes.forEach(function(e){h+=obsCard(e);});h+='</div>';}
    }
  }
  h+='</div>';
  return h;
...
// Sort (sign off) an observation: lined out, stamped with the user's initial + an optional note.
// Stays visible ~30 days then collapses behind the "Older sorted" dropdown.
function _obsUserInitial(){
  if(typeof _rCode==='function'&&S.user)return _rCode(S.user);
  return ((S.user&&S.user.name)||'').split(/\s+/).map(function(w){return w[0]||'';}).join('').toUpperCase().slice(0,2);
}
window.maintObsSort=function(id){
  if(!_maintGuard())return;
  var ac=S._obsAc,e=((S.maintObs||{})[ac]||[]).find(function(x){return x.id===id;});
  if(!e)return;
  var el=document.getElementById('obs-note-'+id);var note=el?el.value.trim():'';
  e.status='resolved';
  e.resolvedBy=(S.user&&S.user.name)||'';
  e.resolvedInitial=_obsUserInitial();
  e.resolvedAt=new Date().toISOString();
  if(note)e.resolvedNote=note;
  _saveMaintObs();
  if(typeof auditLog==='function')auditLog('maint_obs_sort',{ac:ac,id:id});
  render();
};
window.maintObsReopen=function(id){
  if(!_maintGuard())return;
  var ac=S._obsAc,e=((S.maintObs||{})[ac]||[]).find(function(x){return x.id===id;});
  if(!e)return;
  e.status=(e.type==='defect')?'open':'';
  e.resolvedBy=null;e.resolvedAt=null;e.resolvedInitial=null;e.resolvedNote=null;e.resolvedVia=null;
  _saveMaintObs();render();
};
