...
function renderLsSeatGrid(f,a){
  ...
  layout.forEach(function(row){
    ...
    row.forEach(function(cell){
      ...
      // Seatmap-style bubbles: clean card background + a group-colour LEFT accent + dark bold name
      // (instead of a fully group-tinted box) — easier to read at a glance.
      var bgCol=isSelSeat?'rgba(245,158,11,.45)':isPIC?'rgba(59,130,246,.18)':isCoPilot?'rgba(100,116,139,.12)':
        isOccupied?'var(--card2)':(isDropTarget?'rgba(124,58,237,.08)':'var(--card2)');
      var borderCol=isSelSeat?'#f59e0b':isPIC?'#3b82f6':isCoPilot?'#64748b':
        isOccupied?'var(--border2)':(isDropTarget?'var(--accent)':'var(--border2)');
      var accentCol=(isOccupied&&!isPIC&&!isCoPilot)?(gc||'#7c3aed'):null;   // group-colour left accent
      var nameCol=isPIC?'#93c5fd':isCoPilot?'#94a3b8':'var(--text1)';
...
;      var _dragSrc=(_lsMode==='move'&&isOccupied&&!isPIC&&!isCoPilot)?' draggable="true" ondragstart="window.lsSeatDragStart('+idx+',event)"':'';
      ...
      seatsCol+='<div onclick="'+_clickFn+'"'+_dragAttrs+' style="'
        +'width:'+sW+'px;height:'+sH+'px;border-radius:10px;border:2px solid '+borderCol+';'+(accentCol?'border-left:5px solid '+accentCol+';':'')
      ...
      if(displayNm){
        seatsCol+='<div style="font-size:'+(isMob?'11':'12.5')+'px;font-weight:800;color:'+nameCol+';'
          +'line-height:1.15;max-width:'+(sW-12)+'px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'
          +'margin-top:'+(isPIC||isCoPilot?'2':'6')+'px">'+esc(isPIC||isCoPilot?displayNm:displayNm.split(' ')[0])+'</div>';
      ...
      if(isOccupied&&!isPIC&&!isCoPilot&&parseFloat(wt)>0){
        ...
        seatsCol+='<div style="font-size:'+(isMob?'9':'10')+'px;font-weight:700;color:var(--text2);margin-top:2px">'+esc(String(wt))+'kg'+(parseFloat(bg)>0?' +🎒':'')+'</div>';
      }
...
// Collapsed (default) list of every seated passenger with their seat number — quick read for pilots.
function _lsPaxList(f,a){
  if(!a||!a.seats)return '';
  var rows=[];
  a.seats.forEach(function(s,idx){
    var nm=(idx===0?(f.names[0]||f.pic||''):(f.names[idx]||'')).trim();
    if(!nm)return;
    var role=idx===0?'PIC':(idx===1&&f.coPilot?'CP':(s.lbl||('Seat '+idx)));
    var infN=(f.infantNames&&f.infantNames[idx])?String(f.infantNames[idx]).trim():'';
    var isChild=idx>0&&(f.paxType||{})[idx]==='C';
    rows.push('<div style="display:flex;gap:10px;align-items:baseline;padding:4px 0;border-top:1px solid var(--border2)">'+
      '<span style="min-width:48px;font-size:12px;font-weight:800;color:var(--text3);flex-shrink:0">'+esc(role)+'</span>'+
      '<span style="font-size:13.5px;font-weight:600;color:var(--text1)">'+esc(nm)+
        (isChild?'<span style="font-size:10px;font-weight:800;color:#fb923c;margin-left:6px">CHILD</span>':'')+
        (infN?'<span style="font-size:11px;color:#ec4899;margin-left:6px">👶 '+esc(infN)+'</span>':'')+
      '</span></div>');
  });
  if(!rows.length)return '';
  var open=!!S._lsPaxListOpen;
  return '<div class="card" style="padding:9px 13px;margin-top:8px">'+
    '<button onclick="event.stopPropagation();S._lsPaxListOpen=!S._lsPaxListOpen;render()" style="width:100%;display:flex;align-items:center;justify-content:space-between;gap:8px;background:none;border:none;cursor:pointer;color:var(--text1);font-size:13px;font-weight:800;padding:2px 0">'+
      '<span>👥 Passenger list ('+rows.length+')</span><span style="color:var(--text3);font-weight:700">'+(open?'▴ hide':'▾ show')+'</span></button>'+
    (open?'<div style="margin-top:6px">'+rows.join('')+'</div>':'')+
  '</div>';
}
    ...
    const paxCards=a.seats.slice(1).map(function(s,i){
      ...
      return`<div onclick="window.tapFormSeat(${idx},'${f.ac}',event)" draggable="${nm?'true':'false'}" ondragstart="window.lsSeatDragStart(${idx},event)" ondragover="event.preventDefault();event.currentTarget.style.outline='2px solid var(--accent)'" ondragleave="event.currentTarget.style.outline=''" ondrop="window.lsDropOnSeat(${idx},event)" style="position:relative;cursor:${selUnalloc||nm?'pointer':'default'};background:${selSeat?'rgba(245,158,11,.4)':selUnalloc&&!nm?'rgba(124,58,237,.15)':'var(--card2)'};border-radius:7px;padding:4px 6px;border:${payReq?'2px solid #ef4444':selSeat?'2px solid #f59e0b':selUnalloc&&!nm?'2px dashed var(--accent)':'1px solid var(--border2)'};outline:${selSeat?'3px solid rgba(245,158,11,.3)':'none'};border-left:5px solid ${nm?(gc||'var(--border2)'):'var(--border2)'};transition:outline .1s">
        ...
        <div style="display:flex;align-items:center;gap:3px;flex-wrap:wrap;margin-bottom:2px">
          ...
          ${nm
            ?`<div onclick="event.stopPropagation();window.lsNamePopup(${idx})" style="font-size:12.5px;font-weight:800;color:var(--text1);cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:120px">${esc(displayNm.split(' ')[0])}</div>`
  ...
  <div class="card" style="border-left:4px solid ${AC_COL[f.ac]||'var(--accent)'};flex:1;display:flex;flex-direction:column">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      <div style="background:var(--card2);border-radius:10px;padding:10px 12px;border:1px solid var(--border2);cursor:pointer;position:relative" onclick="var i=this.querySelector('input[type=date]');try{i.showPicker&&i.showPicker()}catch(e){i.click()}">
        ...
        <input type="date" class="fi" value="${f.date}" onchange="S.form.date=this.value;autoSaveLS();this.blur();render()" onclick="event.stopPropagation();try{this.showPicker&&this.showPicker()}catch(e){}" style="position:absolute;inset:0;width:100%;height:100%;opacity:0;border:none;background:transparent;cursor:pointer;z-index:10;touch-action:manipulation">
      </div>
