  ...
  
  {
    ...
    if(ua&&ua.length){
      ...
      const hint=selIdx!=null
        ...
        :'<div style="font-size:11px;color:var(--text3);margin-bottom:6px">Tap a passenger, then tap an empty seat above to place them</div>';
  ...
  <div class="card" style="border-left:4px solid ${AC_COL[f.ac]||'var(--accent)'};flex:1;display:flex;flex-direction:column">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      <div style="background:var(--card2);border-radius:10px;padding:10px 12px;border:1px solid var(--border2);cursor:pointer;position:relative" onclick="var i=this.querySelector('input[type=date]');try{i.showPicker&&i.showPicker()}catch(e){i.click()}">
        ...
        <div style="font-size:13px;font-weight:600;color:var(--text1)">${_fmtLsDate(f.date)}</div>
        <input type="date" class="fi" value="${f.date}" onchange="S.form.date=this.value;autoSaveLS();safeRender()" onclick="event.stopPropagation()" style="position:absolute;right:0;bottom:0;width:1px;height:1px;opacity:0;border:none;background:transparent">
      </div>
