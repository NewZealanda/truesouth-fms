...
function _rzPilotsForAc(acId){
  var avail={};_rzAvailablePilots().forEach(function(p){avail[p.code]=1;});
  var out=[],seen={};
  (S.crew||[]).forEach(function(cr){
    ...
    var code=(typeof _rCode==='function'&&u)?_rCode(u):(cr.code?String(cr.code).toUpperCase():(cr.n||'').slice(0,2).toUpperCase());
    if(!code||seen[code])return;seen[code]=1;
    out.push({code:code,name:cr.n||code,weight:cr.w||(u&&u.weight)||0,rostered:!!avail[code]});
  });
