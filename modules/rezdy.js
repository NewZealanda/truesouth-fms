...
function _rzAvailablePilots(){
  ...
  return (S.users||[]).filter(function(u){
    ...
    if(u.role!=='pilot'&&!u.isPilot)return false;
...
function _rzRenderBookings(){
  ...
  // Split the day's bookings by departure time, cancelled to the bottom.
  const allRows=(S._rezdyBookings||[]).slice();
  const deps=[];allRows.forEach(function(b){if(/cancel/i.test(b.status||''))return;var d=_rzBookingDep(b);if(deps.indexOf(d)<0)deps.push(d);});
  deps.sort();
  const depFilter=(S._bkDepFilter&&deps.indexOf(S._bkDepFilter)>=0)?S._bkDepFilter:null;
  const rows=allRows.filter(function(b){return !depFilter||_rzBookingDep(b)===depFilter;}).sort(function(a,b){
    var ca=/cancel/i.test(a.status||'')?1:0,cb=/cancel/i.test(b.status||'')?1:0;if(ca!==cb)return ca-cb;
    return String(_rzBookingDep(a)).localeCompare(String(_rzBookingDep(b)));
  });
  ...
  deps.forEach(function(d){var cnt=allRows.filter(function(b){return !/cancel/i.test(b.status||'')&&_rzBookingDep(b)===d;}).length;depTabs+='<button onclick="S._bkDepFilter=\''+_rzEsc(d).replace(/'/g,"\\'")+'\';render()" class="sub-tab '+(depFilter===d?'on':'')+'" style="font-size:12px;padding:5px 12px">🛫 '+_rzEsc(d)+' <span style="opacity:.6">('+cnt+')</span></button>';});
  depTabs+='</div>';
  const hdr='<div class="card" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">'+
    '<div><div class="st" style="margin-bottom:0">Rezdy Bookings</div>'+
      '<p style="font-size:12px;color:var(--text3);margin:2px 0 0">'+(allRows.length?allRows.length+' booking'+(allRows.length===1?'':'s')+' for '+_rzDowLabel(S.rezdyDate):'No cached bookings')+'</p></div>'+
    '</div>'+depTabs;
  if(!rows.length){
    return hdr+'<div class="card" style="text-align:center;padding:36px;color:var(--text3);font-size:13px">'+(allRows.length?'No bookings at this departure.':'No bookings cached for this date.<br>Tap <b>Refresh from Rezdy</b> to pull the latest.')+'</div>';
  }
  ...
  const NCOL=10; // chevron+order, customer, pax, product, departs, pickup, phone, status, balance, comments (email hidden until expand)
  let body='';
  let _curDep=null;
  rows.forEach(function(b){
    // Group header when the departure time changes (a clean split between 0930, 1200, …).
    var _bdep=/cancel/i.test(b.status||'')?'Cancelled':_rzBookingDep(b);
    if(_bdep!==_curDep){
      _curDep=_bdep;
      var _grp=rows.filter(function(x){return (/cancel/i.test(x.status||'')?'Cancelled':_rzBookingDep(x))===_bdep;});
      var _gbd={a:0,c:0,i:0};_grp.forEach(function(x){var e=_rzEffBreakdown(x);_gbd.a+=e.a;_gbd.c+=e.c;_gbd.i+=e.i;});
      var _prod=_rzProduct((((_grp[0]||{}).items||[])[0]||{}).product||'');
      body+='<tr><td colspan="'+NCOL+'" style="padding:9px 8px;background:var(--card2);border-top:2px solid var(--border)"><span style="font-size:13px;font-weight:800;color:var(--text1)">'+(_bdep==='Cancelled'?'✕ Cancelled':'🛫 '+_rzEsc(_bdep))+'</span><span style="font-size:11px;color:var(--text3);font-weight:600;margin-left:10px">'+_grp.length+' booking'+(_grp.length===1?'':'s')+' · '+_rzBdText(_gbd)+(_prod&&_bdep!=='Cancelled'?' · '+_rzEsc(_prod):'')+'</span></td></tr>';
    }
    ...
    if(_bub){
      var _ovr=_rzHasPaxOverrides(ono);
      var _rst=_ovr?'<button onclick="window.rezdyResetPax(\''+_rzEsc(ono).replace(/'/g,"\\'")+'\')" title="Reset this booking to the Rezdy standard" style="flex-shrink:0;align-self:center;margin-left:auto;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.04em;color:var(--text3);background:rgba(255,255,255,.05);border:1px solid var(--border2);border-radius:6px;padding:3px 9px;cursor:pointer;white-space:nowrap">↺ Reset</button>':'';
      body+='<tr><td colspan="'+NCOL+'" style="padding:0;border-top:1px dashed var(--border2);background:var(--card2)"><div style="display:flex;align-items:stretch">'+_bub+_rst+'</div></td></tr>';
    }
    if(!/cancel/i.test(b.status||'')){
      body+='<tr><td colspan="'+NCOL+'" style="padding:0;background:var(--card2)">'+_rzBookingAcPills(b,ono)+'</td></tr>';
    }
    if(open){
    ...
    // Gap between bookings so each one is clearly separated.
    body+='<tr aria-hidden="true"><td colspan="'+NCOL+'" style="height:12px;padding:0;background:transparent;border-bottom:2px solid var(--border)"></td></tr>';
  });
  ...
  const table='<div class="card" style="overflow-x:auto;padding:10px"><table style="width:100%;border-collapse:collapse;font-size:12.5px;color:var(--text)">'+
    '<thead><tr>'+
      ...
      '<th style="'+th+'">Phone</th><th style="'+th+'">Status</th><th style="'+th+'">Balance</th><th style="'+th+'">Comments</th>'+
...
window.rezdyLoadPickups=async function(){
  ...
  if(row&&Array.isArray(row.vans)){
    ...
    S._pickupVans=null;S._pickupCollected={};S._pickupLocOverride={};S._pickupDrivers=[];S._pickupExtraDrivers=[];S._rezdyPaxMeta={};S._schedPilots={};S._rzBookingAc={};
  }
...
function _rzRenderManifest(){
  ...
  var _mPilots=_rzAvailablePilots();
  if(_mPilots.length){
...
window.rezdyManDropPilot=function(acId,e){
  ...
  var en=_rzPilotEndorse(code);
  if(en&&en.length&&en.indexOf(acId)<0){if(typeof toast==='function')toast(code+' is not type-rated on '+String(acId).replace('ZK-','')+'.','warn');return;}
...
window.rezdyManCreateLoadsheet=function(acId){
  ...
  if(picCode){
    var pu=_rzAvailablePilots().find(function(x){return x.code===picCode;});
    var picName=pu?pu.name:'';
    if(picName){form.pic=picName;form.names[0]=picName;var cr=(typeof anyCrewList==='function'?anyCrewList():[]).find(function(c){return c.n===picName;});if(cr&&cr.w)form.seats[0]=String(cr.w);}
  }
