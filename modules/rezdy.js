...
// Short product code → full customer-facing flight-type name (shown on the weather link / wx.html).
var _RZ_PROD_NAME={
  FLB:'Milford Sound One-Way Flight', CCF:'Milford Sound Coach-Cruise-Fly', CCC:'Milford Sound Coach-Cruise-Coach',
  FCF:'Milford Sound Fly-Cruise-Fly', FEF:'Milford Sound Fly-Explore-Fly', MFOH:'Milford Sound Scenic Flight',
  MCEXP:'Aoraki / Mount Cook Expedition', MCGL:'Mount Cook Helicopter Glacier Landing', MCOH:'Mount Cook Scenic Flight',
  MCHS:'Mount Cook Heli-Ski', STT:'Ski the Tasman', THH:'Tasman Heli Hike',
  FJHH:'Franz Josef Glacier Heli-Hike', FJGL:'Franz Josef Helicopter Glacier Landing', FJOH:'Franz Josef Scenic Flight',
  ASP:'Mount Aspiring Scenic Flight', QNLS:'Queenstown Local Scenic Flight', BRA:'Branches Station', CHT:'Charter Flight'
};
function _rzProductName(p){var code=_rzProduct(p);return _RZ_PROD_NAME[code]||code||'';}
...
window.rezdyNewBookingOpen=function(){S._rzNewBkDraft={name:'',phone:'',email:'',time:'',product:'FCF',a:1,c:0,i:0,ac:'',pickup:'',owing:'',note:'',adt:[],chd:[],inf:[]};render();};
...
// Per-passenger name + DECLARED weight (kind: 'adt'|'chd'|'inf'). No render → keeps input focus.
window.rezdyNewBkPax=function(kind,idx,field,val){var d=S._rzNewBkDraft;if(!d)return;d[kind]=d[kind]||[];while(d[kind].length<=idx)d[kind].push({});d[kind][idx][field]=val;};
...
window.rezdyNewBookingSave=function(){
  ...
  var owing=parseFloat(d.owing);owing=(isFinite(owing)&&owing>0)?owing:0;
  // Native (in-house) bookings live in ts_native_bookings — durable, cross-device, and coexisting with
  // Rezdy. Fall back to the legacy per-date pickup blob only if the platform store isn't available.
  var useNative=(typeof platformSaveBooking==='function'&&typeof platformNewOrderNo==='function');
  var order=useNative?platformNewOrderNo():('M-'+Date.now().toString(36).toUpperCase().slice(-6));
  ...
  // Participants carry the DECLARED name + weight (raw stated kg; the app adds the +4kg allowance via
  // _rzDeclared, same as Rezdy). Order: adults, children, infants — matching the form rows.
  var participants=[];
  for(var _pa=0;_pa<a;_pa++){var _px=(d.adt&&d.adt[_pa])||{};participants.push({name:String(_px.name||'').trim(),weight:String(_px.wt||'').trim(),age:''});}
  for(var _pc=0;_pc<c;_pc++){var _px2=(d.chd&&d.chd[_pc])||{};participants.push({name:String(_px2.name||'').trim(),weight:String(_px2.wt||'').trim(),age:'Child'});}
  for(var _pi=0;_pi<i;_pi++){var _px3=(d.inf&&d.inf[_pi])||{};participants.push({name:String(_px3.name||'').trim(),weight:'',age:'Infant'});} // infants are lap pax — no weight
  // Pickup location → flows into Transport (_rzPickups reads it.pickup + it.pickupTime). The pickup
  // time = departure minus the location's "minutes before" from the master list.
  var _pk=String(d.pickup||'').trim(),_pkTime='';
  if(_pk){
    var _loc=(S._rzPickupLocs||[]).find(function(L){return String(L.name||'')===_pk;});
    var _min=_loc?(parseInt(_loc.min,10)||0):0;
    var _pm=(parseInt(tm.slice(0,2),10)*60+parseInt(tm.slice(2),10))-_min;if(_pm<0)_pm=0;
    _pkTime=S.rezdyDate+'T'+String(Math.floor(_pm/60)).padStart(2,'0')+':'+String(_pm%60).padStart(2,'0')+':00';
  }
  var booking={id:order,orderNumber:order,status:'CONFIRMED',customerName:name,phone:String(d.phone||'').trim(),email:String(d.email||'').trim(),comments:String(d.note||'').trim(),fields:{},source:'Manual',
    totalPax:a+c+i,totalAmount:owing,totalPaid:0,balanceDue:owing,currency:'NZD',
    items:[{product:prod,startTimeLocal:S.rezdyDate+'T'+hhmm+':00',quantity:a+c+i,quantities:quantities,pickup:_pk,pickupTime:_pkTime,extras:[],participants:participants}],
    _tourDate:S.rezdyDate};
  if(useNative)booking._native=true;else booking._manual=true;
  ...
  if(useNative){platformSaveBooking(booking);}   // → ts_native_bookings (sbU queues + retries if offline)
  else{S._rzManualBk=S._rzManualBk||[];S._rzManualBk.push(booking);}
  if(window.pickupSave)window.pickupSave(true);   // persists the aircraft assignment (+ manual blob on the legacy path)
  if(typeof toast==='function')toast(useNative?'Booking added ✓':'Booking added','ok');render();
};
// Delete an in-house booking (native or legacy-manual — Rezdy-synced bookings can't be deleted here).
window.rezdyDeleteManualBooking=function(order){
  order=String(order);
  if(!confirm('Delete this booking?'))return;
  var b=(S._rezdyBookings||[]).find(function(x){return String((x&&(x.orderNumber||x.id))||'')===order;});
  var td=(b&&b._tourDate)||S.rezdyDate;
  S._rzManualBk=(S._rzManualBk||[]).filter(function(x){return String(x.orderNumber||'')!==order;});
  S._rezdyBookings=(S._rezdyBookings||[]).filter(function(x){return String(x.orderNumber||'')!==order;});
  ...
  if(b&&b._native&&typeof platformDeleteBooking==='function')platformDeleteBooking(order,td);   // remove from ts_native_bookings
  if(window.pickupSave)window.pickupSave(true);
  if(typeof toast==='function')toast('Booking deleted','info');render();
};
// Re-save a native booking after an in-place edit (balance / source) so the change reaches the store.
function _rzPersistNativeIfAny(order){var b=(S._rezdyBookings||[]).find(function(x){return x&&x._native&&String((x.orderNumber||x.id))===String(order);});if(b&&typeof platformSaveBooking==='function')platformSaveBooking(b);}
function _rzNewBookingModal(){
  ...
  if(typeof _rzPickupLocsLoad==='function')_rzPickupLocsLoad();var _bkLocs=Array.isArray(S._rzPickupLocs)?S._rzPickupLocs:[];
  ...
  var h='<div onclick="window.rezdyNewBookingCancel()" class="rzci-overlay">'+
    ...
    (function(){
      var na=parseInt(d.a,10)||0,nc=parseInt(d.c,10)||0,ni=parseInt(d.i,10)||0;
      if(na+nc+ni===0)return '';
      var wst='font-size:15px;padding:7px 8px;background:var(--card2);color:var(--text);border:1px solid var(--border2);border-radius:7px;outline:none';
      var row=function(kind,idx,lbl,withWt){var px=(d[kind]||[])[idx]||{};
        return '<div style="display:flex;gap:6px;align-items:center;margin-bottom:5px">'+
          '<span style="font-size:10px;font-weight:800;color:var(--text3);width:54px;flex-shrink:0">'+lbl+'</span>'+
          '<input type="text" value="'+_rzEsc(px.name||'')+'" oninput="window.rezdyNewBkPax(\''+kind+'\','+idx+',\'name\',this.value)" placeholder="Name" style="'+wst+';flex:1;min-width:0">'+
          (withWt?'<input type="number" inputmode="decimal" min="0" step="1" value="'+_rzEsc(px.wt||'')+'" oninput="window.rezdyNewBkPax(\''+kind+'\','+idx+',\'wt\',this.value)" placeholder="kg" style="'+wst+';width:64px;flex-shrink:0">':'<span style="width:64px;flex-shrink:0;font-size:10px;color:var(--text3);text-align:center">lap</span>')+
        '</div>';};
      var h='<label style="font-size:11px;color:var(--text3);font-weight:700">DECLARED WEIGHTS <span style="font-weight:400;text-transform:none">— optional; a pre-check-in W&amp;B estimate (+4kg allowance added automatically). Actuals are captured at check-in.</span></label><div style="margin:4px 0 10px">';
      for(var k=0;k<na;k++)h+=row('adt',k,'Adult '+(k+1),true);
      for(var k=0;k<nc;k++)h+=row('chd',k,'Child '+(k+1),true);
      for(var k=0;k<ni;k++)h+=row('inf',k,'Infant '+(k+1),false);
      return h+'</div>';
    })()+
    '<label style="font-size:11px;color:var(--text3);font-weight:700">PICKUP LOCATION (optional)</label>'+
    '<select onchange="window.rezdyNewBkField(\'pickup\',this.value)" style="'+fst+';margin:2px 0 10px"><option value="">— No pickup / self-drive —</option>'+_bkLocs.map(function(L){var nm=String(L.name||'');if(!nm)return '';return '<option value="'+_rzEsc(nm)+'"'+(d.pickup===nm?' selected':'')+'>'+_rzEsc(nm)+(L.min?(' · '+L.min+'m before'):'')+'</option>';}).join('')+'</select>'+
    '<div style="display:flex;gap:8px;margin-bottom:10px">'+
      ...
      '<div style="flex:1"><label style="font-size:11px;color:var(--text3);font-weight:700">EMAIL (optional)</label><input type="email" value="'+_rzEsc(d.email||'')+'" oninput="window.rezdyNewBkField(\'email\',this.value)" placeholder="Email" style="'+fst+';margin-top:2px"></div>'+
    '</div>'+
    '<div style="display:flex;gap:8px;margin-bottom:10px">'+
      ...
      '<div style="flex:1"><label style="font-size:11px;color:var(--text3);font-weight:700">AMOUNT OWING $ (optional)</label><input type="number" inputmode="decimal" min="0" step="0.01" value="'+_rzEsc(d.owing||'')+'" oninput="window.rezdyNewBkField(\'owing\',this.value)" placeholder="0.00 = paid" style="'+fst+';margin-top:2px"></div>'+
    '</div>'+
    '<label style="font-size:11px;color:var(--text3);font-weight:700">NOTE (optional)</label>'+
    '<textarea oninput="window.rezdyNewBkField(\'note\',this.value)" placeholder="Internal note — special requests, agent, etc." style="'+fst+';margin:2px 0 14px;min-height:44px;resize:vertical">'+_rzEsc(d.note||'')+'</textarea>'+
...
function _rzOrdersForBlockKey(key){
  var out=[];var attach=S._rzSchedAttach||{};
  (S._rezdyBookings||[]).forEach(function(b){
    if(_rzIsCancelled(b))return;
    var order=String(b.orderNumber||'');
    // A booking COMBINED (manually attached) into a target block belongs to THAT block, not its own
    // departure slot — so block ops (reassign / change-aircraft) move the whole combined group together
    // instead of stranding the attached riders (which then fell back to auto-allocation). v28.17
    if(attach[order]){if(attach[order]===key)out.push(order);return;}
    var ac=_rzBookingAc(b,order)||'__unalloc__';
    ((b.items)||[]).forEach(function(it){var t=_rzDepTime(it.startTimeLocal||'');if(!t)return;var start=_rzHHMMcolon(t);var prod=_rzProduct(it.product);if(_rzDepKey(ac,start,prod)===key)out.push(order);});
  });
  return out;
}
...
function _wxSnapFor(b,ono){
  ...
  return {pax_name:b.customerName||'',dep_key:depKey,dep_time:depTime,dep_label:prodName||destName||'',aircraft:acLbl,fr_date_label:dlab,
          ...
          is_self_drive:(/self.?drive|walk.?in|own (transport|car|vehicle)/i.test(loc||''))};
}
...
function _rzRenderWeatherCalls(){
  ...
  // The calendar PIC pins (S._schedPilots) live in the pickup blob. The seatmap loads it; the Weather
  // page did NOT — so opening Weather without the blob loaded left S._schedPilots empty and the PIC fell
  // back to AUTO (showing e.g. TP instead of the calendar-set JY). Load it here too. (v28.17)
  if(!S._rzManPickupSynced&&window.rezdyLoadPickups){S._rzManPickupSynced=true;window.rezdyLoadPickups();}
...
// Edit an in-house booking's balance owing / source. Native → re-saved to ts_native_bookings;
// legacy-manual → persisted in the pickup blob via manualBk.
window.rezdyManualSetBalance=function(order,val){order=String(order);var n=parseFloat(val);n=isFinite(n)&&n>0?n:0;
  [S._rzManualBk,S._rezdyBookings].forEach(function(arr){(arr||[]).forEach(function(x){if(x&&(x._manual||x._native)&&String(x.orderNumber||x.id)===order){x.balanceDue=n;x.totalAmount=n;}});});
  _rzPersistNativeIfAny(order);if(window.pickupSave)window.pickupSave(true);render();};
window.rezdyManualSetSource=function(order,val){order=String(order);var s=String(val||'').trim()||'Manual';
  [S._rzManualBk,S._rezdyBookings].forEach(function(arr){(arr||[]).forEach(function(x){if(x&&(x._manual||x._native)&&String(x.orderNumber||x.id)===order)x.source=s;});});
  _rzPersistNativeIfAny(order);if(window.pickupSave)window.pickupSave(true);render();};
...
function _rzBookingCard(b){
  ...
  var wxAuto=(typeof _wxLinkAcked==='function')&&_wxLinkAcked(ono);                  // customer acknowledged via their link (shown on the 🔗 button)
  ...
  var _pkPhoneH=(function(){
    ...
    var locs=[],ov=S._pickupLocOverride||{},tov=S._pickupTimeOverride||{},ptime='';(b.items||[]).forEach(function(it,ii){if(!it.pickup)return;var pid=ono+'|'+(it.product||'')+'|'+(it.startTimeLocal||'')+'|'+ii;var l=(ov[pid]!=null&&ov[pid]!=='')?ov[pid]:it.pickup;l=String(l||'').trim();if(l&&locs.indexOf(l)<0)locs.push(l);if(!ptime){var tt=(tov[pid]!=null&&tov[pid]!=='')?tov[pid]:((typeof _rzDepTime==='function')?_rzDepTime(it.pickupTime||''):'');ptime=tt?((typeof _rzHHMMcolon==='function')?_rzHHMMcolon(tt):tt):'';}});
    var isSelf=(_wxAct==='self_drive');
    var locStr=isSelf?'Self drive':locs.join(', ');     // customer chose self-drive → show Self drive
    var locH=locStr?'<span title="Pickup">📍 '+_rzEsc(locStr)+((!isSelf&&ptime)?' · '+_rzEsc(ptime):'')+'</span>':'';
  ...
  if(!cancelled){
    h+=_rzCheckBtn('Wx',wx,'window.rezdyBookingToggleWx(\''+oE+'\')','Weather check — manual staff tick');
    var _wxEx=(typeof _wxLinkExists==='function')&&_wxLinkExists(ono);
    h+='<button onclick="event.stopPropagation();window.wxLinkOpen(\''+oE+'\')" title="Customer weather link — copy, status &amp; history" style="display:inline-flex;align-items:center;gap:3px;font-size:11px;font-weight:700;padding:4px 9px;border-radius:14px;cursor:pointer;white-space:nowrap;border:1.5px solid '+(wxAuto?'#22c55e':(_wxEx?'#60a5fa':'var(--border2)'))+';background:'+(wxAuto?'rgba(34,197,94,.12)':'transparent')+';color:'+(wxAuto?'#16a34a':(_wxEx?'#60a5fa':'var(--text3)'))+'">🔗 Wx link</button>';
