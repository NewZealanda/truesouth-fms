...
function _rzRenderPickups(){
  ...
  S._pickupVans.forEach(function(vanIds,vi){
    ...
    vansH+='<div style="margin-bottom:10px">'+
      ...
      '</div>'+
      (_dpOpen?'<div style="position:relative;z-index:30;margin-top:4px;border:1px solid var(--acc);border-radius:8px;background:var(--card);max-height:280px;overflow-y:auto;box-shadow:0 10px 28px rgba(0,0,0,.45)">'+_rzDriverPickRows('window.pickupSetVanDriver('+vi+',%N%)')+'</div>':'')+
      '</div>';
    var _depJs=_rzEsc(depFilter||'').replace(/'/g,"\\'");
      ...
      _vlist.forEach(function(id){
        ...
        vansH+='<div draggable="true" ondragstart="window.pickupDragStart(\''+_rzEsc(id).replace(/'/g,"\\'")+'\',event)" '+
          ...
          '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px">'+
            '<div style="font-weight:700;font-size:13px;color:var(--text);min-width:0;'+(collected?'text-decoration:line-through':'')+'"><span style="color:var(--text3);font-weight:700;margin-right:6px;cursor:grab;font-size:17px;vertical-align:middle" title="Drag to reorder">≡</span>'+_rzEsc(p.customer||p.order)+'</div>'+
...
function _rzDriverHasUnacked(){
  ...
  var has=false;(S._pickupVans||[]).forEach(function(ids,vi){if(((S._pickupDrivers||[])[vi]||'').trim()===me&&!_rzVanAcked(vi))has=true;});
  return has;
}
setInterval(function(){
  try{
...
function _rzAllDriverNames(){
  var out=[];[].concat(_rzAvailableDrivers(),(S._pickupExtraDrivers||[]),(S._pickupDrivers||[]).filter(Boolean)).forEach(function(n){n=(n||'').trim();if(n&&out.indexOf(n)<0)out.push(n);});
  return out;
}
    ...
    grp.forEach(function(p){
      ...
      h+='<div draggable="true" ondragstart="window.myPkDragStart(\''+_idD+'\',event)" ondragover="event.preventDefault()" ondrop="window.myPkDrop('+_viD+',\''+_depD+'\',\''+_idD+'\',event)" style="background:'+(_isNew?'rgba(34,197,94,.10)':'var(--card2)')+';border:'+(_isNew?'2px solid #22c55e':'1px solid var(--border2)')+';border-left:4px solid '+(_isNew?'#22c55e':col)+';border-radius:10px;padding:13px;margin-bottom:10px;cursor:grab;'+(collected?'opacity:.55':'')+'">'+
        '<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:6px">'+
          '<div style="font-weight:800;font-size:16px;color:var(--text1);min-width:0;'+(collected?'text-decoration:line-through':'')+'"><span style="color:var(--text3);font-weight:700;margin-right:8px;cursor:grab;font-size:20px;vertical-align:middle" title="Drag to reorder">≡</span>'+_rzEsc(p.customer||p.order)+(_isNew?' <span style="font-size:10px;font-weight:900;color:#06281a;background:#22c55e;padding:1px 6px;border-radius:10px;vertical-align:middle">NEW</span>':'')+'</div>'+
