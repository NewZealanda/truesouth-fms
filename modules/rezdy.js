...
function _rzBookingDetail(b){
  ...
  var srcH='<div style="margin-bottom:10px"><div style="'+sec+'">Source</div><span class="pill" style="background:var(--card2);border:1px solid var(--border2);color:var(--text2);font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px">'+_rzEsc(_rzSourceLabel(b))+'</span>';
  // Diagnostic (superadmin only): show the raw source-related fields so we can find the agent name.
  if(b._srcDbg&&S.user&&S.user.role==='superadmin'){
    srcH+='<div style="font-size:9px;color:var(--text3);word-break:break-all;margin-top:4px;font-family:monospace">DBG '+_rzEsc(JSON.stringify(b._srcDbg))+'</div>';
  }
  srcH+='</div>';
