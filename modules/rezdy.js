...
function _rzRenderBookings(){
  ...
  var _ciCount=active.filter(function(b){return (S._rzBookingCheckedIn||{})[String(b.orderNumber||'')];}).length;
  const hdr='<div class="card" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">'+
    ...
    '<button class="btn btn-primary" style="font-size:12px;padding:7px 12px'+(_ciCount?'':';opacity:.55')+'" title="Send all checked-in passengers to the Manifest for seat allocation" onclick="window.rezdyPushToManifest()">⤒ Push to manifest'+(_ciCount?' ('+_ciCount+')':'')+'</button>'+
    '</div>';
  ...
  let body='';
  if(searching){
    // Search overrides the departure filter — match across every booking on the date.
    var matches=allRows.filter(function(b){return _rzBkMatch(b,q);})
      .sort(function(a,b){return _rzBookingDep(a).localeCompare(_rzBookingDep(b));});
    body+='<div style="margin:16px 0 8px;display:flex;align-items:baseline;gap:10px;flex-wrap:wrap"><span style="font-size:15px;font-weight:800;color:var(--text1)">🔍 '+matches.length+' result'+(matches.length===1?'':'s')+'</span><span style="font-size:11px;color:var(--text3);font-weight:600">for “'+_rzEsc(q)+'”</span></div>';
    if(!matches.length)body+='<div class="card" style="text-align:center;padding:30px;color:var(--text3);font-size:13px">No bookings match “'+_rzEsc(q)+'”.</div>';
    else matches.forEach(function(b){body+=_rzBookingCard(b);});
    return top+body;
  }
...
function _rzRenderManifest(){
  ...
  fleet.forEach(function(id){
    ...
    h+='<div class="rzman-card" ondragover="event.preventDefault();this.style.outline=\'2px solid '+col+'\'" ondragleave="this.style.outline=\'\'" ondrop="this.style.outline=\'\';window.rezdyManDrop(\''+id+'\',event)" style="background:var(--card);border:1px solid var(--border);border-top:3px solid '+col+';border-radius:10px;padding:12px">'+
...
// Push the day's checked-in bookings into the Manifest (from the Bookings tab). Loads the saved
// manifest first (so manual pax / seating aren't clobbered), then runs the same pull.
...
window.rezdyManPull=function(){
  ...
  (S._rezdyBookings||[]).forEach(function(b){
    ...
    ci.forEach(function(r,i){
      ...
      out.push({id:id,name:r.name||'',weight:wv,declaredWeight:useDecl,type:isInf?'adult':(r.type==='child'?'child':'adult'),infantName:null,group:order,paymentReq:owing,ac:ex?ex.ac:null,acHint:acHint,infantOf:isInf?(r.attach!=null?order+'|c'+r.attach:undefined):(ex?ex.infantOf:undefined)});
    });
  });
  // Lap infants follow their host's aircraft (they aren't seated).
  out.forEach(function(p){if(p.infantOf){var host=out.find(function(x){return x.id===p.infantOf;});if(host)p.ac=host.ac;}});
  ...
  rows.forEach(function(r){
    ...
    r.forEach(function(cell){
      ...
      if(p){
        ...
        var tbc=!p.weight;var decld=!tbc&&!!p.declaredWeight;
        var wTxt=tbc?'TBC':(_rzEsc(String(p.weight))+'kg'+(decld?' (d)':''));
        var wCol=tbc?'#b45309':(decld?'#dc2626':'#15803d');
