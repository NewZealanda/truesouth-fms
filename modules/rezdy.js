...
function _rzRenderManifest(){
  ...
  fleet.forEach(function(id){
    ...
    if(picCode){
      ...
      h+='<div ondragover="event.preventDefault();event.stopPropagation();this.style.outline=\'2px solid #818cf8\'" ondragleave="this.style.outline=\'\'" ondrop="event.stopPropagation();this.style.outline=\'\';window.rezdyManDropPilot(\''+idE+'\',event)" style="display:flex;align-items:center;gap:6px;margin-bottom:6px;padding:5px 8px;border-radius:7px;border:1px dashed '+(_coCode?'#818cf8':'var(--border2)')+';background:'+(_coCode?'rgba(129,140,248,.1)':'transparent')+'">'+
...
// Natural (un-combined) departure for a manifest pax.
function _rzBaseDep(p){if(!p)return '—';if(p._dep)return p._dep;var i=_rzManDepInfo(p.group);return (i&&i.dep)||'—';}
...
window.rezdyManClearCoPic=function(acId){if(S._rzManCoPic)delete S._rzManCoPic[acId];_rzManReseatDepForAc(acId);_rzManSave();render();};
...
window.rezdyOpenLsTab=function(id){
  ...
  if(s){_rzLsTabAdd(id,(s.form&&s.form.ac)||'',_rzLsTabLabel((s.form&&s.form.ac)||'',s.form));if(typeof window.viewSaved==='function')window.viewSaved(id);return;}
  try{fetch(SB+'/rest/v1/ts_loadsheets?id=eq.'+encodeURIComponent(id)+'&select=*',{headers:SH}).then(function(r){return r.ok?r.json():[];}).then(function(rows){
    var row=rows&&rows[0];
    if(row&&row.form){S.saved=S.saved||[];S.saved.push({id:row.id,form:row.form,status:row.status||'unsigned',savedAt:row.saved_at});_rzLsTabAdd(id,row.form.ac||'',_rzLsTabLabel(row.form.ac||'',row.form));if(typeof window.viewSaved==='function')window.viewSaved(id);}
...
// ── Rezdy Loadsheets sub-tab — a shared, live list of open loadsheet tabs for this date ──
function _rzRenderLoadsheets(){
  ...
  var h='<div class="card"><div class="st" style="margin-bottom:4px">Loadsheets</div>'+
    '<p style="font-size:12px;color:var(--text3);margin:0 0 8px">'+_rzDowLabel(S.rezdyDate)+' · everyone viewing this date shares the same open loadsheet tabs. Create from the <b>Seatmap</b>, start a blank below, or re-open one from Saved.</p>'+
  ...
  h+='</div></div>';
  // The shared list of open loadsheet tabs.
  ...
  if(!tabs.length){
    h+='<div style="font-size:13px;color:var(--text3);padding:8px 0">No open loadsheets for this day yet. Create one from the Seatmap, start a blank above, or open a saved one below.</div>';
  }else{
    tabs.forEach(function(t){
      var s=(S.saved||[]).find(function(x){return x.id===t.id;});
      var signed=s&&(s.status==='complete'||s.status==='signed'||(s.form&&s.form.sig));
      var ac=(s&&s.form&&s.form.ac)||t.ac||'';var col=_rzAcCol(ac);
      var etd=(s&&s.form&&s.form.etd)||'';
      var idE=String(t.id).replace(/'/g,"\\'");
  ...
  h+='</div>';
  // Saved loadsheets for this date that aren't currently in the open-tabs list (re-open to share).
