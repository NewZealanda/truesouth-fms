...
window.rezdyBookingNoShow=function(order){
  ...
  if(S._rzNoShow[order])delete S._rzNoShow[order];else S._rzNoShow[order]=true;
  if(window.pickupSave)window.pickupSave(true);render();
};
...
function _rzBookingCard(b){
  ...
  if(!cancelled){
    ...
    h+='<button onclick="window.rezdyBookingNoShow(\''+oE+'\')" title="No-show — shows red and is excluded from the seatmap push" style="display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:700;padding:4px 9px;border-radius:14px;cursor:pointer;white-space:nowrap;border:1.5px solid '+(noShow?'#ef4444':'var(--border2)')+';background:'+(noShow?'rgba(239,68,68,.15)':'transparent')+';color:'+(noShow?'#f87171':'var(--text3)')+'">'+(noShow?'✕':'○')+' No-show</button>';
...
window.rezdySetDate=function(v){
  ...
  S._rzBookingCheckedIn={};S._rzBookingAc={};S._rzBookingWx={};S._pickupLocOverride={};S._rezdyPaxMeta={};S._rzCheckin={};S._rzSchedAttach={};S._rzManDepMerge={};S._schedPilots={};S._rzBookingCancel={};S._rzNoShow={};
  render();
